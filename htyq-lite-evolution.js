// htyq-lite-evolution.js — 演化API调用 + 事件链强制触发 + 血仇自动追杀
// ============================================================
// ★ 修改记录 ★
// 2026-06-05 v2.1.0
...
window.HTYQ_LITE_EVOLUTION = (function() {
  ...
  async function callApi(prompt, maxTokens = 2000, temperature = 0.7) {
    ...
    if (apiMode === 'tavern') {
      ...
      try {
        ...
        console.warn('[HTYQ] Tavern API 调用失败，尝试回退自定义 API:', err.message);
        if (customUrl) {
  ...
  function forceTriggerEvents(state) {
    ...
    for (const ev of state.events) {
      ...
      if (remaining <= 0 && ev.status !== '已爆发') {
        ...
        console.log(`[HTYQ Evolution] 🚨 强制触发事件: ${ev.name}`);
        ...
        ev.stage = '即将爆发';
        ev.currentRound++;
        console.log(`[HTYQ Evolution] ⚠️ 事件即将爆发: ${ev.name}`);
      } else if (remaining > 0 && ev.stage !== '已爆发' && ev.stage !== '挂起') {
        ev.currentRound++;
        if (remaining <= 2) {
          // 剩余 2 轮进入发酵阶段
          if (ev.stage === '萌芽') ev.stage = '发酵';
          console.log(`[HTYQ Evolution] 事件推进: ${ev.name} (${ev.currentRound}/${ev.totalRounds})`);
        }
      }
    }
    ...
    for (const bf of state.bloodFeudMemo) {
      ...
      if (state.round >= bf.nextAttackRound) {
        ...
        if (bf.attackCount >= 5) {
          ...
          console.log(`[HTYQ Evolution] 🔪 血仇追杀 #${bf.attackCount}: ${bf.faction}`);
        }
        advanced = true;
  ...
  async function callEvolutionAPI(state, userMsg, aiMsg, retryCount = 0) {
    ...
    } catch(e) { console.warn('[HTYQ] 获取聊天历史失败', e); }
    ...
    var core2 = window.HTYQ_LITE_CORE;
    var storyBlock = '';
...
注意：宁可生成合理的小变化，也不要脱离正文编造大变化。如果你觉得"变化很少"，就输出轻量更新，而不是强行制造戏剧化转折。`;
    ...
    try {
      ...
      if (validation.warnings.length > 0) {
        console.warn('[HTYQ Evolution] 演化验证:', validation.warnings.join('; '));
      }
      update = validation.fixedUpdate;

      // 如果无效且未重试过，重试一次（temperature 提高）
      if (!validation.valid && retryCount < 1) {
        console.warn('[HTYQ Evolution] 演化验证失败，尝试重试...');
  ...
  function decayRumors(state) {
    ...
    if (toRemove.length || toMutate.length) {
      core.saveState(state);
      console.log(`[HTYQ Evolution] 流言老化：移除 ${toRemove.length} 条，异变 ${toMutate.length} 条，剩余 ${state.rumors.length} 条`);
    }
  }
  ...
  async function evolve(state, userMsg, aiMsg) {
    ...
    if (apiSuccess) {
      ...
      core.saveState(state);
      console.warn('[HTYQ Evolution] 演化失败，状态已回滚');
      return false;
    }
  }
