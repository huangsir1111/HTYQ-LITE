/* htyq-lite-ui.js — 全面 UI 重写 v3.0.0
 * 基于预览设计，替换全部 ?? 占位符，全 emoji 化，数据驱动
 * ────────────────────────────────────────────────────── */
window.HTYQ_LITE_UI = (function() {
  'use strict';

  var core = window.HTYQ_LITE_CORE;
  var memory = window.HTYQ_LITE_MEMORY;
  var evolution = window.HTYQ_LITE_EVOLUTION;
  var worldbook = window.HTYQ_LITE_WORLDBOOK;
  var timeModule = window.HTYQ_LITE_TIME;
  ...
  function toast(msg, err, dur) {
    ...
    el.className = 'htyq-lite-toast' + (err ? ' error' : '');
    el.textContent = msg;
  ...
  function showPersistToast(msg, err) {
    var id = 'htyq-lite-persist-toast';
  ...
  function removePersistToast() { var el = document.getElementById('htyq-lite-persist-toast'); if (el) el.remove(); }
  ...
  function showSemiAutoConfirmDialog(evolvePreviewText, onConfirm, onSkip) {
    ...
    overlay.innerHTML = '<div class="htyq-modal-box"><div class="htyq-modal-hdr">⚠️ 半自动模式预览</div><div class="htyq-modal-body"><p style="margin-bottom:8px;color:#8b949e;font-size:11px;">以下是本轮推演的预览结果，请确认是否执行：</p><div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;padding:10px;font-size:12px;color:#e6edf3;max-height:200px;overflow-y:auto;line-height:1.7;">'+esc(evolvePreviewText)+'</div></div><div class="htyq-modal-actions"><button class="btn btn-success" id="htyq-semi-confirm">✅ 确认执行</button><button class="btn" id="htyq-semi-skip">⏭ 跳过此轮</button><button class="btn btn-danger" id="htyq-semi-cancel">✖ 取消</button></div></div>';
    document.body.appendChild(overlay);
    document.getElementById('htyq-semi-confirm').addEventListener('click', function(){ overlay.remove(); if (onConfirm) onConfirm(); });
    document.getElementById('htyq-semi-skip').addEventListener('click', function(){ overlay.remove(); if (onSkip) onSkip(); });
    document.getElementById('htyq-semi-cancel').addEventListener('click', function(){ overlay.remove(); });
  ...
  function showNotificationPanel() {
    ...
    overlay.innerHTML = '<div class="htyq-modal-box" style="max-width:420px;"><div class="htyq-modal-hdr">🔔 通知历史 <span style="font-size:11px;color:#8b949e;font-weight:400;">最近 '+items.length+'条</span></div><div class="htyq-modal-body" style="max-height:320px;overflow-y:auto;">'+listHtml+'</div><div class="htyq-modal-actions"><button class="btn btn-sm" id="htyq-notif-close" style="margin-left:auto;">✖ 关闭</button></div></div>';
    document.body.appendChild(overlay);
    document.getElementById('htyq-notif-close').addEventListener('click', function(){ overlay.remove(); });
    overlay.addEventListener('click', function(e) { if (e.target === overlay) overlay.remove(); });
  }

  /* ── v3.0.1: 批量操作工具栏 ── */
  function renderBatchActionBar(containerId, onSelectAll, onBatchDelete, onBatchImportance) {
    var html = '<div class="htyq-batch-bar" id="htyq-batch-bar">';
    html += '<label class="tw" style="display:flex;align-items:center;gap:4px;cursor:pointer;"><input type="checkbox" id="htyq-batch-select-all" style="accent-color:#238636;"><span class="sm" style="font-size:11px;">全选</span></label>';
    html += '<span class="sm gray" id="htyq-batch-count">已选 0 条</span>';
    html += '<button class="btn btn-sm btn-danger" id="htyq-batch-delete" disabled style="font-size:10px;padding:2px 8px;">🗑️ 批量删除</button>';
    if (onBatchImportance) html += '<button class="btn btn-sm" id="htyq-batch-importance" disabled style="font-size:10px;padding:2px 8px;">🔥 标记重要性</button>';
    html += '</div>';
    var cont = document.getElementById(containerId);
    if (!cont) return;
    var existing = document.getElementById('htyq-batch-bar');
    if (existing) existing.remove();
    cont.insertAdjacentHTML('afterbegin', html);
    setTimeout(function(){
      var selAll = document.getElementById('htyq-batch-select-all');
      if (selAll) selAll.addEventListener('change', function(){
        var checked = this.checked;
        document.querySelectorAll('.htyq-mem-select').forEach(function(cb) { cb.checked = checked; });
        if (onSelectAll) onSelectAll(checked);
        updateBatchCount();
      });
      document.getElementById('htyq-batch-delete').addEventListener('click', function(){ if (onBatchDelete) onBatchDelete(); });
      if (onBatchImportance) document.getElementById('htyq-batch-importance').addEventListener('click', function(){ if (onBatchImportance) onBatchImportance(); });
    }, 50);
  }
  function updateBatchCount() {
    var checked = document.querySelectorAll('.htyq-mem-select:checked').length;
    var countEl = document.getElementById('htyq-batch-count');
    var delBtn = document.getElementById('htyq-batch-delete');
    var impBtn = document.getElementById('htyq-batch-importance');
  ...
  function addInputBarButton() {
    var existing = document.getElementById('htyq-lite-toggle-btn');
    if (existing) return;
  ...
  function renderOverview(cont, state) {
    ...
    html += '<div id="htyq-wdesc-display-ov" style="background:#0d1117;border-radius:6px;padding:8px;border:1px solid #21262d;font-size:11px;color:#e6edf3;line-height:1.6;">';
    html += '<div>'+(s.worldDescription ? esc(s.worldDescription) : '<span class="gray">(\u6682\u65e0\u7b80\u8ff0)</span>')+'</div></div>';
    html += '<div class="flex mt-8"><button class="btn btn-sm" id="htyq-wdesc-edit-ov">\u270f\ufe0f \u7f16\u8f91\u4e16\u754c\u7b80\u8ff0</button>';
    html += '<button class="btn btn-sm btn-primary" id="htyq-wdesc-save-ov" style="display:none">\ud83d\udcbe \u4fdd\u5b58</button></div>';
    html += '<textarea id="htyq-wdesc-editor-ov" style="display:none;width:100%;min-height:50px;padding:6px;border-radius:6px;border:1px solid #30363d;background:#0d1117;color:#e6edf3;font-size:11px;resize:vertical;box-sizing:border-box;margin-top:4px">'+esc(s.worldDescription||'')+'</textarea></div>';
    ...
    setTimeout(function(){
      var edBtn = document.getElementById('htyq-wdesc-edit-ov');
      var svBtn = document.getElementById('htyq-wdesc-save-ov');
      var ed = document.getElementById('htyq-wdesc-editor-ov');
      ...
      // quick evolve button
      var qeBtn = document.getElementById('htyq-quick-evolve');
      if (qeBtn) {
  ...
  function renderAchievements(cont, state) {
    ...
    html += '</div>';
    html += '<div class="flex"><span class="tw"><label class="tg"><input type="checkbox" id="htyq-ach-show-hidden" '+(showHidden?'checked':'')+'><span class="s"></span></label><span class="sm gray">\u663e\u793a\u9690\u85cf</span></span>';
    html += '<span class="tw"><label class="tg"><input type="checkbox" id="htyq-ach-auto-gen" '+(autoGen?'checked':'')+'><span class="s"></span></label><span class="sm gray">AI \u81ea\u52a8\u751f\u6210</span></span></div></div>';
    ...
    for (var ak in allDefs) {
      ...
      if (achProg && achProg.max > 1 && !isUnlocked) {
        ...
        html += '<div class="htyq-ach-progress"><div class="htyq-ach-progress-bar" style="width:'+pct+'%;background:'+barColor+'"></div></div>';
        html += '<div class="htyq-progress-text">'+achProg.current+' / '+achProg.max+'</div>';
      }
    ...
    // bind ach events
    setTimeout(function(){
      var showCb = document.getElementById('htyq-ach-show-hidden');
      var genCb = document.getElementById('htyq-ach-auto-gen');
      ...
      if (achFilter) achFilter.addEventListener('change', function(){
        ...
        document.querySelectorAll('#htyq-ach-grid .ach-card').forEach(function(c){
          ...
          var showNsfw = document.getElementById('htyq-ach-show-hidden');
      ...
      // \u2605 v3.0.1: \u6210\u5c31\u6392\u5e8f
      var achSort = document.getElementById('htyq-ach-sort');
      if (achSort) achSort.addEventListener('change', function(){
        var v = this.value;
        try { localStorage.setItem('htyq_ach_sort', v); } catch(e){}
      ...
      if (achSearch) achSearch.addEventListener('input', function(){
        ...
        document.querySelectorAll('#htyq-ach-grid .ach-card').forEach(function(c){
          ...
          if (c.style.display !== 'none') {
            var nsfwEl = document.getElementById('htyq-ach-show-hidden');
      ...
      var nsfwInit = document.getElementById('htyq-ach-show-hidden');
  ...
  function renderWorld(cont, state) {
    ...
    if (emEntries.length > 0) {
      for (var ei = 0; ei < emEntries.length; ei++) {
        ...
        // ★ v3.0.1: 生命周期手动下拉
        html += '<select class="htyq-lifecycle-select" data-char="'+esc(cName)+'" data-orig="'+lcKey+'">';
        ...
        html += '<div style="display:flex;gap:4px;"><button class="btn btn-sm htyq-entity-edit" data-char="'+esc(cName)+'" style="font-size:9px;padding:2px 6px;">\u270e \u7f16\u8f91</button></div></div>';
      }
    } else {
    ...
    html += '<div class="fa mt-8"><button class="btn btn-sm" id="htyq-create-faction">\u2795 \u521b\u5efa\u52bf\u529b</button></div></div>';
    ...
    html += '<div class="flex mt-8"><button class="btn btn-sm" id="htyq-wdesc-edit">\u270f\ufe0f \u7f16\u8f91\u4e16\u754c\u7b80\u8ff0</button>';
    html += '<button class="btn btn-sm btn-primary" id="htyq-wdesc-save" style="display:none">\ud83d\udcbe \u4fdd\u5b58</button></div>';
    html += '<textarea id="htyq-wdesc-editor" style="display:none;width:100%;min-height:50px;padding:6px;border-radius:6px;border:1px solid #30363d;background:#0d1117;color:#e6edf3;font-size:11px;resize:vertical;box-sizing:border-box;margin-top:4px">'+esc(state.worldDescription||'')+'</textarea></div></div>';
    ...
    if (npcLog.length) {
      ...
      for (var ni = 0; ni < npcLog.length && shown < 8; ni++) {
        ...
        if (!a || !a.npc) continue;
        shown++;
        html += '<div class="tl-i"><div class="rnd">#'+a.round+'</div><div class="txt">'+esc(a.npc)+'\uff1a'+esc(a.activity)+(a.location&&a.location!=='\u672a\u77e5'?'\uff08'+esc(a.location)+'\uff09':'')+'</div><button class="htyq-sched-edit-btn" data-npc="'+esc(a.npc)+'" data-activity="'+esc(a.activity)+'" data-location="'+esc(a.location||'')+'" style="flex-shrink:0;margin-left:4px;">\u270f</button></div>';
      }
    } else {
    ...
    html += '<div class="card"><div class="card-title" style="display:flex;align-items:center;gap:6px;">\ud83d\udde3\ufe0f \u8c23\u8a00\u4e0e\u4f20\u95fb <span class="bdg">'+rumors.length+'</span><button class="btn btn-sm htyq-rumor-add" style="margin-left:auto;font-size:9px;padding:2px 6px;">\u2795 \u6dfb\u52a0\u8c23\u8a00</button></div>';
    if (rumors.length) {
      html += '<div class="tl">';
    ...
    html += '</div>';
    html += '<div class="fa"><button class="btn btn-sm btn-primary" id="htyq-worldlaw-nav">\ud83c\udf10 \u67e5\u770b\u5b8c\u6574\u4e16\u754c\u6cd5\u5219</button></div></div>';

    cont.innerHTML = html;

    // bind world desc editor
    setTimeout(function(){
      var edBtn = document.getElementById('htyq-wdesc-edit');
      var svBtn = document.getElementById('htyq-wdesc-save');
      var disp = document.getElementById('htyq-wdesc-display');
      var ed = document.getElementById('htyq-wdesc-editor');
      if (!edBtn || !svBtn || !ed) return;
      edBtn.addEventListener('click', function(){
        ed.style.display = 'block'; edBtn.style.display = 'none'; svBtn.style.display = 'inline-block'; ed.focus();
      });
      ...
      // create faction
      var cfBtn = document.getElementById('htyq-create-faction');
      ...
      // create event
      var ceBtn = document.getElementById('htyq-create-event');
      ...
      document.querySelectorAll('.htyq-event-advance').forEach(function(btn){
        btn.addEventListener('click', function(){
      ...
      document.querySelectorAll('.htyq-entity-edit').forEach(function(btn){
        btn.addEventListener('click', function(){
      ...
      // ★ v3.0.1: 生命周期下拉变更
      document.querySelectorAll('.htyq-lifecycle-select').forEach(function(sel){
      ...
      // ★ v3.0.1: NPC日程编辑
      document.querySelectorAll('.htyq-sched-edit-btn').forEach(function(btn){
        btn.addEventListener('click', function(){
      ...
      // rumor add
      document.querySelectorAll('.htyq-rumor-add').forEach(function(btn){
        btn.addEventListener('click', function(){
      ...
      // rumor delete
      document.querySelectorAll('.htyq-rumor-delete').forEach(function(btn){
        btn.addEventListener('click', function(){
      ...
      // world law nav
      var wlBtn = document.getElementById('htyq-worldlaw-nav');
      if (wlBtn) wlBtn.addEventListener('click', function(){
        switchTab('worldbook');
        // scroll to the world law editor area
        setTimeout(function(){
          var wlSection = document.getElementById('htyq-wl-section');
  ...
  function renderMemory(cont, state) {
    ...
    html += '<div id="htyq-memory-batch-area"></div>';
    html += '<div class="card"><div class="card-title">\ud83d\udcbe \u8bb0\u5fc6\u5217\u8868</div><div id="htyq-memory-list">';
    ...
    if (recent.length === 0) {
      ...
      for (var mi = 0; mi < recent.length; mi++) {
        ...
        html += '<input type="checkbox" class="htyq-mem-select" data-mem-round="'+(m.round||'')+'" style="flex-shrink:0;accent-color:#238636;">';
        ...
        html += '<button class="btn btn-sm htyq-mem-edit" data-mem-round="'+(m.round||'')+'" style="font-size:9px;padding:0 4px;">\u270e</button>';
        html += '<button class="btn btn-sm htyq-mem-del" data-mem-round="'+(m.round||'')+'" style="font-size:9px;padding:0 4px;color:#f85149;">\u2716</button>';
        html += '<button class="btn btn-sm htyq-mem-imp" data-mem-round="'+(m.round||'')+'" style="font-size:9px;padding:0 4px;" title="\u4fee\u6539\u91cd\u8981\u6027">\ud83d\udd25</button></span>';
        html += '</div>';
      }
    }
    html += '</div></div>';
    ...
    html += '<span class="tag tag-ent" style="cursor:pointer;" onclick="document.getElementById(\'htyq-mem-search\').value=\'\u5b9e\u4f53\';document.getElementById(\'htyq-mem-search\').dispatchEvent(new Event(\'input\'));">\u5b9e\u4f53 '+tagCounts.ent+'</span> ';
    html += '<span class="tag tag-loc" style="cursor:pointer;" onclick="document.getElementById(\'htyq-mem-search\').value=\'\u5730\u70b9\';document.getElementById(\'htyq-mem-search\').dispatchEvent(new Event(\'input\'));">\u5730\u70b9 '+tagCounts.loc+'</span> ';
    html += '<span class="tag tag-fac" style="cursor:pointer;" onclick="document.getElementById(\'htyq-mem-search\').value=\'\u52bf\u529b\';document.getElementById(\'htyq-mem-search\').dispatchEvent(new Event(\'input\'));">\u52bf\u529b '+tagCounts.fac+'</span> ';
    html += '<span class="tag tag-top" style="cursor:pointer;" onclick="document.getElementById(\'htyq-mem-search\').value=\'\u4e3b\u9898\';document.getElementById(\'htyq-mem-search\').dispatchEvent(new Event(\'input\'));">\u4e3b\u9898 '+tagCounts.top+'</span> ';
    html += '<span class="tag tag-emo" style="cursor:pointer;" onclick="document.getElementById(\'htyq-mem-search\').value=\'\u60c5\u611f\';document.getElementById(\'htyq-mem-search\').dispatchEvent(new Event(\'input\'));">\u60c5\u611f '+tagCounts.emo+'</span>';
    html += '</div></div>';
    ...
    // bind memory search
    setTimeout(function(){
      var searchInp = document.getElementById('htyq-mem-search');
      var memList = document.getElementById('htyq-memory-list');
      ...
      // \u8bb0\u5fc6\u7b5b\u9009
      var memFilter = document.getElementById('htyq-mem-filter');
      if (memFilter) memFilter.addEventListener('change', function(){
        var v = this.value;
        document.querySelectorAll('#htyq-memory-list .mem-item').forEach(function(item){
      ...
      if (typeof renderBatchActionBar === 'function') {
        renderBatchActionBar('htyq-memory-batch-area',
          function(checked){ /* selectAll callback - auto handled by renderBatchActionBar */ },
          function(){
            // batch delete
            var selected = [];
            document.querySelectorAll('.htyq-mem-select:checked').forEach(function(cb){
        ...
        document.querySelectorAll('.htyq-mem-select').forEach(function(cb){
          cb.addEventListener('change', function(){ if (typeof updateBatchCount === 'function') updateBatchCount(); });
        });
      }
      // ★ v3.0.1: 阈值保存
      var saveThreshBtn = document.getElementById('htyq-save-thresholds');
      if (saveThreshBtn) {
        saveThreshBtn.addEventListener('click', function(){
          var hotEl = document.getElementById('htyq-hot-threshold');
          var archEl = document.getElementById('htyq-archive-threshold');
      ...
      var custBtn = document.getElementById('htyq-save-custom-entities');
      if (custBtn) custBtn.addEventListener('click', function(){
        var inp = document.getElementById('htyq-custom-entities');
      ...
      // manual add memory
      var memBtn = document.getElementById('htyq-add-memory');
      ...
      // memory edit
      document.querySelectorAll('.htyq-mem-edit').forEach(function(btn){
      ...
      // memory delete
      document.querySelectorAll('.htyq-mem-del').forEach(function(btn){
        btn.addEventListener('click', function(ev){
          ev.stopPropagation();
          var round = parseInt(this.getAttribute('data-mem-round'));
          if (isNaN(round)) { toast('\u26a0\ufe0f \u65e0\u6548\u8bb0\u5fc6', true); return; }
      ...
      document.querySelectorAll('.htyq-mem-imp').forEach(function(btn){
        btn.addEventListener('click', function(ev){
          ev.stopPropagation();
          var round = parseInt(this.getAttribute('data-mem-round'));
          if (isNaN(round)) { toast('\u26a0\ufe0f \u65e0\u6548\u8bb0\u5fc6', true); return; }
  ...
  function renderEngine(cont, state) {
    ...
    html += '<div class="fg"><label>\u5f15\u64ce\u6a21\u5f0f</label><select id="htyq-drive-mode">';
    ...
    html += '</select></div>';
    html += '<div class="fg"><label>\u63a8\u6f14\u95f4\u9694\uff08\u624b\u52a8/\u534a\u81ea\u52a8\uff09</label><select id="htyq-evolve-interval">';
    [1,3,5,10].forEach(function(n){ html += '<option value="'+n+'"'+(evolveInterval==n?' selected':'')+'>\u6bcf '+n+' \u6761\u6d88\u606f</option>'; });
    html += '</select></div></div>';
    html += '<div class="fa"><button class="btn btn-primary" id="htyq-save-drive">\ud83d\udcbe \u4fdd\u5b58\u9a71\u52a8\u8bbe\u7f6e</button></div></div>';
    ...
    html += '<div class="fg"><label>\u5f53\u524d\u65f6\u95f4\u6587\u672c</label><input type="text" id="htyq-time-text" value="'+esc(timeStr)+'"></div>';
    html += '<div class="fg"><label>\u7eaa\u5143</label><input type="text" id="htyq-era" value="'+esc(state.era||'\u7eaa\u5143 1247 \u5e74')+'"></div></div>';
    html += '<div class="fr"><div class="fg"><label>\u6bcf\u8f6e\u65f6\u95f4\u589e\u91cf</label><select id="htyq-time-inc">';
    ...
    html += '</select></div>';
    html += '<div class="fg"><label>\u65e5\u591c\u5faa\u73af</label><select id="htyq-daynight"><option selected>\u5f00\u542f</option><option>\u5173\u95ed</option></select></div></div>';
    html += '<div class="fg"><label>\ud83d\udd50 \u5feb\u6377\u65f6\u95f4\u9884\u8bbe\uff08\u4e00\u952e\u5e94\u7528\uff09</label>';
    html += '<div class="flex" style="gap:4px;flex-wrap:wrap;">';
    html += '<button class="btn btn-sm htyq-preset-time" data-min="15">\u26a1 \u7d27\u8feb</button>';
    html += '<button class="btn btn-sm htyq-preset-time" data-min="60">\u2600\ufe0f \u6b63\u5e38</button>';
    html += '<button class="btn btn-sm htyq-preset-time" data-min="240">\ud83c\udf19 \u7f13\u6162</button>';
    html += '<button class="btn btn-sm htyq-preset-time" data-min="720">\ud83d\udd01 \u65e5\u00b7\u591c\u6a21\u5f0f</button>';
    html += '<button class="btn btn-sm htyq-preset-time" data-min="10080">\ud83c\udf31 \u5b63\u8282\u6a21\u5f0f</button>';
    html += '<button class="btn btn-sm htyq-preset-time" data-min="43200">\u23e9 \u5feb\u901f\u63a8\u8fdb</button>';
    html += '</div></div>';
    html += '<div class="fa"><button class="btn btn-primary" id="htyq-save-time">\ud83d\udcbe \u4fdd\u5b58\u65f6\u95f4\u8bbe\u7f6e</button></div></div>';

    // evolution settings
    html += '<div class="card"><div class="card-title">\ud83e\uddec \u6f14\u5316\u8bbe\u7f6e</div><div class="fr">';
    html += '<div class="fg"><label>NPC \u6d3b\u52a8\u9891\u7387</label><select id="htyq-npc-freq"><option>\u4f4e</option><option selected>\u4e2d</option><option>\u9ad8</option></select></div>';
    html += '<div class="fg"><label>\u4e16\u754c\u4e8b\u4ef6\u6982\u7387</label><select id="htyq-ev-prob"><option>\u4f4e (10%)</option><option selected>\u4e2d (30%)</option><option>\u9ad8 (50%)</option></select></div></div>';
    html += '<div class="fr"><div class="fg"><label>\u60c5\u611f\u8870\u51cf\u901f\u5ea6</label><select id="htyq-emotion-decay"><option>\u6162 (10 \u8f6e)</option><option selected>\u6807\u51c6 (5 \u8f6e)</option><option>\u5feb (3 \u8f6e)</option></select></div>';
    html += '<div class="fg"><label>\u89d2\u8272\u751f\u547d\u5468\u671f</label><select id="htyq-lifecycle"><option selected>\u5f00\u542f</option><option>\u5173\u95ed</option></select></div></div>';
    html += '<div class="fg"><label>\u81ea\u52a8\u5267\u60c5\u9636\u6bb5\u63a8\u8fdb</label><select id="htyq-phase-auto"><option selected>\u5f00\u542f (\u6bcf 10 \u8f6e\u81ea\u52a8\u63a8\u8fdb)</option><option>\u5173\u95ed</option></select></div>';
    html += '<div class="fa"><button class="btn btn-primary" id="htyq-save-evolve">\ud83d\udcbe \u4fdd\u5b58\u6f14\u5316\u8bbe\u7f6e</button>';
    html += '<button class="btn btn-success" id="htyq-manual-evolve">\u25b6\ufe0f \u624b\u52a8\u89e6\u53d1\u4e00\u8f6e\u63a8\u6f14</button>';
    html += '<button class="btn btn-danger" id="htyq-emergency-stop" style="'+(driveMode==='ai'||driveMode==='auto'?'':'display:none')+'">\u26a0\ufe0f \u7d27\u6025\u505c\u6b62 \u4e00\u952e\u7981\u6b62\u81ea\u52a8\u6f14\u5316</button></div></div>';

    // injection style
    html += '<div class="card"><div class="card-title">\ud83d\udc89 \u6ce8\u5165\u98ce\u683c <span class="bdg">A/B \u6d4b\u8bd5</span></div><div class="fr">';
    html += '<div class="fg"><label>\u5f53\u524d\u98ce\u683c</label><select id="htyq-inject-style">';
    ['\u6807\u51c6\u53d9\u4e8b\u98ce','\u6781\u7b80\u98ce','\u6c89\u6d78\u5f0f\uff08\u542b\u4e16\u754c\u7ec6\u8282\uff09','\u6218\u6597\u5f3a\u5316','\u89d2\u8272\u626e\u6f14\u6781\u81f4','\u60ac\u5ff5\u63a2\u7d22','\u9ed1\u6697\u5199\u5b9e','\u8bd7\u610f\u6d6a\u6f2b','\u5e7d\u9ed8\u641e\u7b11','\u81ea\u5b9a\u4e49'].forEach(function(s){ html += '<option'+(s==='\u6c89\u6d78\u5f0f\uff08\u542b\u4e16\u754c\u7ec6\u8282\uff09'?' selected':'')+'>'+s+'</option>'; });
    html += '</select></div>';
    html += '<div class="fg"><label>\u6ce8\u5165\u4f18\u5148\u7ea7</label><select id="htyq-inject-priority">';
    ['\u81ea\u52a8\uff08\u667a\u80fd\u4f18\u5148\uff09','\u4e8b\u4ef6 > \u60c5\u611f > \u8bb0\u5fc6','\u60c5\u611f > \u4e8b\u4ef6 > \u8bb0\u5fc6','\u8bb0\u5fc6 > \u60c5\u611f > \u4e8b\u4ef6'].forEach(function(p){ html += '<option'+(p==='\u4e8b\u4ef6 > \u60c5\u611f > \u8bb0\u5fc6'?' selected':'')+'>'+p+'</option>'; });
    html += '</select></div></div>';
    html += '<div class="fr"><div class="fg"><label>Token \u9884\u7b97</label><input type="number" id="htyq-token-budget" value="4096"></div>';
    html += '<div class="fg"><label>\u667a\u80fd\u6298\u53e0\u7ea7\u522b</label><select id="htyq-fold-level"><option selected>\u667a\u80fd\uff08\u81ea\u52a8 3 \u7ea7\u6298\u53e0\uff09</option><option>\u5bbd\u677e\uff08\u4fdd\u7559\u66f4\u591a\u5185\u5bb9\uff09</option><option>\u4e25\u683c\uff08\u4fdd\u7559\u6838\u5fc3\u5185\u5bb9\u53ea\uff09</option></select></div></div>';
    html += '<div class="fa"><button class="btn btn-primary" id="htyq-save-inject">\ud83d\udcbe \u4fdd\u5b58\u6ce8\u5165\u98ce\u683c</button></div></div>';
    // ★ v3.0.1: 自定义注入模板
    html += '<div class="card"><div class="card-title">\ud83d\udcdd \u81ea\u5b9a\u4e49\u6ce8\u5165\u6a21\u677f <span class="bdg">\u8986\u76d6\u7cfb\u7edf\u9ed8\u8ba4</span></div>';
    html += '<textarea id="htyq-custom-inject-template" style="width:100%;min-height:60px;padding:6px;border-radius:6px;border:1px solid #30363d;background:#0d1117;color:#e6edf3;font-size:11px;resize:vertical;box-sizing:border-box;" placeholder="\u8f93\u5165\u81ea\u5b9a\u4e49\u6ce8\u5165\u6a21\u677f\u5185\u5bb9... \u7559\u7a7a\u4f7f\u7528\u7cfb\u7edf\u9ed8\u8ba4\u3002\u652f\u6301 {era} {time} {world} {characters} {memories} {events} \u53d8\u91cf\u3002">' + esc(state.customInjectTemplate || '') + '</textarea>';
    html += '<div class="fa"><button class="btn btn-sm btn-primary" id="htyq-save-custom-inject">\ud83d\udcbe \u4fdd\u5b58\u81ea\u5b9a\u4e49\u6a21\u677f</button><button class="btn btn-sm" id="htyq-clear-custom-inject" style="margin-left:4px;">\ud83d\uddd1\ufe0f \u6e05\u9664</button></div></div>';
    ...
    setTimeout(function(){
      // time presets
      document.querySelectorAll('.htyq-preset-time').forEach(function(btn){
        btn.addEventListener('click', function(){
          var min = parseInt(this.dataset.min);
          var incSel = document.getElementById('htyq-time-inc');
      ...
      // save drive mode
      var saveDrive = document.getElementById('htyq-save-drive');
      if (saveDrive) {
        saveDrive.addEventListener('click', function(){
          var dm = document.getElementById('htyq-drive-mode');
          var intr = document.getElementById('htyq-evolve-interval');
          var st = core.loadState();
      ...
      // save time
      var saveTime = document.getElementById('htyq-save-time');
      if (saveTime) {
        saveTime.addEventListener('click', function(){
          var st = core.loadState();
          var timeTextEl = document.getElementById('htyq-time-text');
          var eraEl = document.getElementById('htyq-era');
          var timeIncEl = document.getElementById('htyq-time-inc');
          var dayNightEl = document.getElementById('htyq-daynight');
      ...
      // save evolve
      var saveEvolve = document.getElementById('htyq-save-evolve');
      if (saveEvolve) {
        saveEvolve.addEventListener('click', function(){
          var npcFreqEl = document.getElementById('htyq-npc-freq');
          var evProbEl = document.getElementById('htyq-ev-prob');
          var emotionDecayEl = document.getElementById('htyq-emotion-decay');
          var lifecycleEl = document.getElementById('htyq-lifecycle');
          var phaseAutoEl = document.getElementById('htyq-phase-auto');
          var s = JSON.parse(localStorage.getItem('htyq_lite_settings') || '{}');
      ...
      // save inject
      var saveInject = document.getElementById('htyq-save-inject');
      if (saveInject) {
        saveInject.addEventListener('click', function(){
          var styleEl = document.getElementById('htyq-inject-style');
          var priorityEl = document.getElementById('htyq-inject-priority');
          var tokenEl = document.getElementById('htyq-token-budget');
          var foldEl = document.getElementById('htyq-fold-level');
          var s = JSON.parse(localStorage.getItem('htyq_lite_settings') || '{}');
      ...
      // manual evolve
      var manualBtn = document.getElementById('htyq-manual-evolve');
      if (manualBtn) {
        ...
        manualBtn.addEventListener('click', function(){
          ...
          if (driveMode === 'semi') {
            ...
            showSemiAutoConfirmDialog(preview,
              function(){
                // confirm - do evolve
                var btn = document.getElementById('htyq-manual-evolve');
                btn.disabled = true; btn.textContent = '\u23f3 \u63a8\u6f14\u4e2d...';
                executeEvolve(btn);
              },
              function(){
      ...
      var emStop = document.getElementById('htyq-emergency-stop');
      if (emStop) {
  ...
  function renderStory(cont, state) {
    ...
    html += '<div class="fg"><label>\u6545\u4e8b\u6a21\u677f\uff0812 \u79cd\u7ecf\u5178\u53d9\u4e8b\u6a21\u5f0f\uff09</label><select id="htyq-story-template">';
    templates.forEach(function(t){ html += '<option value="'+t.id+'"'+(t.name===storyTemplate || t.id===storyTemplate?' selected':'')+'>'+t.name+'</option>'; });
    html += '</select></div>';
    html += '<div class="fg"><label>\u60c5\u611f\u57fa\u8c03\uff088 \u79cd\u53d9\u4e8b\u6c1b\u56f4\uff09</label><select id="htyq-story-tone">';
    ...
    html += '<div class="fg"><label>\u81ea\u5b9a\u4e49\u57fa\u8c03\u6587\u672c\uff08\u9009\u81ea\u5b9a\u4e49\u65f6\u751f\u6548\uff09</label><input type="text" id="htyq-custom-tone" placeholder="\u8f93\u5165\u81ea\u5b9a\u4e49\u60c5\u611f\u57fa\u8c03\u63cf\u8ff0..." value="'+esc((state.storyType&&state.storyType.customToneText)||'')+'"></div></div>';
    html += '<div class="flex-sb mt-8"><span class="tw"><label class="tg"><input type="checkbox" id="htyq-phase-auto-toggle" '+(enableAuto?'checked':'')+'><span class="s"></span></label><span class="sm gray">\u81ea\u52a8\u63a8\u8fdb\u9636\u6bb5\uff08\u6bcf 10 \u8f6e\uff09</span></span>';
    html += '<button class="btn btn-primary" id="htyq-save-story">\ud83d\udcbe \u4fdd\u5b58\u6545\u4e8b\u8bbe\u7f6e</button></div>';
    ...
    if (threads.length) {
      ...
      for (var pi = 0; pi < Math.min(threads.length, 6); pi++) {
        ...
        html += '<button class="btn btn-sm mt-8 htyq-thread-advance" data-thread-id="'+esc(t.id||t.name||pi)+'">\ud83d\udccc \u63a8\u8fdb\u9636\u6bb5</button>';
        html += '</div>';
      }
      html += '</div>';
    } else {
    ...
    html += '<div class="fa"><button class="btn btn-sm" id="htyq-new-thread">\u2795 \u65b0\u5efa\u5267\u60c5\u7ebf\u7d22</button></div></div>';
    ...
    if (pKeys.length > 0) {
      ...
      for (var pi2 = 0; pi2 < Math.min(pKeys.length, 8); pi2++) {
        ...
        html += '<div class="port-card htyq-portrait-card" data-char="'+esc(pKeys[pi2])+'"><div style="font-size:26px;">'+(p.emoji||'\ud83d\udc64')+'</div>';
        ...
        html += '<div style="display:flex;gap:2px;margin-top:4px;justify-content:center;"><button class="btn btn-sm htyq-portrait-edit" data-char="'+esc(pKeys[pi2])+'" style="font-size:9px;padding:1px 4px;">\u270e</button>';
        html += '<button class="btn btn-sm htyq-portrait-del" data-char="'+esc(pKeys[pi2])+'" style="font-size:9px;padding:1px 4px;color:#f85149;">\u2716</button></div>';
        html += '</div>';
      }
      html += '</div>';
    } else {
    ...
    html += '<div class="fa"><button class="btn btn-sm" id="htyq-create-portrait">\u2795 \u624b\u52a8\u521b\u5efa\u753b\u50cf</button></div></div>';
    ...
    html += '<span class="flex" style="gap:4px;"><button class="btn btn-sm" id="htyq-combat-add" style="font-size:9px;padding:2px 6px;">\u2795 \u6dfb\u52a0\u6218\u6597\u65e5\u5fd7</button>';
    html += '<button class="btn btn-sm" id="htyq-combat-clear" style="font-size:9px;padding:2px 6px;color:#f85149;">\ud83d\uddd1\ufe0f \u6e05\u7a7a\u65e5\u5fd7</button></span></div></div>';
    ...
    // bind events
    setTimeout(function(){
      var saveStory = document.getElementById('htyq-save-story');
      if (saveStory) {
        saveStory.addEventListener('click', function(){
          var tmpl = document.getElementById('htyq-story-template');
          var tone = document.getElementById('htyq-story-tone');
          var autoCb = document.getElementById('htyq-phase-auto-toggle');
          var customTone = document.getElementById('htyq-custom-tone');
          var st = core.loadState();
      ...
      // \u65b0\u5efa\u5267\u60c5\u7ebf\u7d22
      var nt = document.getElementById('htyq-new-thread');
      ...
      // \u624b\u52a8\u521b\u5efa\u753b\u50cf
      var cp = document.getElementById('htyq-create-portrait');
      ...
      // \u91cd\u65b0\u5206\u6790
      var an = document.getElementById('htyq-analyze');
      ...
      // \u63a8\u8fdb\u5267\u60c5\u7ebf\u7d22\u9636\u6bb5
      document.querySelectorAll('.htyq-thread-advance').forEach(function(btn){
        btn.addEventListener('click', function(){
      ...
      // portrait edit
      document.querySelectorAll('.htyq-portrait-edit').forEach(function(btn){
        btn.addEventListener('click', function(){
          var charName = this.getAttribute('data-char');
          if (!charName) return;
          var st = core.loadState();
      ...
      // portrait delete
      document.querySelectorAll('.htyq-portrait-del').forEach(function(btn){
        btn.addEventListener('click', function(){
          var charName = this.getAttribute('data-char');
          if (!charName) return;
      ...
      // combat add
      var cbAdd = document.getElementById('htyq-combat-add');
      ...
      // combat clear
      var cbClear = document.getElementById('htyq-combat-clear');
  ...
  function renderWorldbook(cont) {
    var _aa = localStorage.getItem('htyq_wb_autoActivate') === 'true';
    var html = '';
    html += '<div class="guide-box"><div class="gt">\ud83d\udcda \u4e16\u754c\u4e66\u6d4f\u89c8\u5668</div><div class="gd">\u4e0b\u62c9\u6846\u9009\u4e66 \u2192 \u6fc0\u6d3b \u2192 \u52fe\u9009\u6761\u76ee \u2192 \u4fdd\u5b58\u3002\u7cbe\u7ec6\u63a7\u5236\u54ea\u4e9b\u4e16\u754c\u4e66\u3001\u54ea\u4e9b\u6761\u76ee\u6ce8\u5165 AI \u4e0a\u4e0b\u6587\u3002</div></div>';

    // ===== \u4e0b\u62c9\u6846\u9009\u62e9\u4e16\u754c\u4e66 =====
    html += '<div class="card"><div class="card-title">\ud83d\udcd6 \u9009\u62e9\u4e16\u754c\u4e66 <span class="bdg" id="htyq-wb-selected-count">0 \u672c\u5df2\u9009</span></div>';
    html += '<div class="fg"><label>\u4e16\u754c\u4e66\uff08\u4e0b\u62c9\u9009\u62e9\uff0c\u641c\u7d22\u6846\u53ef\u8fc7\u6ee4\uff09</label>';
    html += '<select id="htyq-wb-select" style="width:100%;padding:6px 10px;background:#0d1117;border:1px solid #30363d;border-radius:6px;color:#e6edf3;font-size:12px;font-family:inherit;">';
    html += '<option value="">\u2014 \u8bf7\u9009\u62e9\u4e16\u754c\u4e66 \u2014</option>';
    html += '</select></div>';
    html += '<div class="flex" style="gap:4px;flex-wrap:wrap;">';
    html += '<button class="btn btn-sm btn-success" id="htyq-wb-activate-selected">\u2705 \u6fc0\u6d3b\u5f53\u524d\u4e66</button>';
    html += '<button class="btn btn-sm btn-danger" id="htyq-wb-deactivate-selected">\u274c \u53d6\u6d88\u5f53\u524d\u4e66</button>';
    html += '<button class="btn btn-sm" id="htyq-wb-activate-all">\u2705 \u5168\u90e8\u6fc0\u6d3b</button>';
    html += '<button class="btn btn-sm" id="htyq-wb-deactivate-all">\u274c \u5168\u90e8\u53d6\u6d88</button>';
    html += '<button class="btn btn-primary" id="htyq-save-wb">\ud83d\udcbe \u4fdd\u5b58\u4e66\u7ea7\u9009\u62e9</button>';
    html += '<button class="btn btn-sm" id="htyq-refresh-wb">\ud83d\udd04 \u5237\u65b0\u5217\u8868</button>';
    html += '</div>';
    html += '<div class="fg mt-8"><label>\u641c\u7d22\u4e16\u754c\u4e66</label><input type="text" id="htyq-wb-search" placeholder="\u8f93\u5165\u4e66\u540d\u4e2d\u65ad\u7eed..." style="width:100%;padding:6px 10px;background:#0d1117;border:1px solid #30363d;border-radius:6px;color:#e6edf3;font-size:12px;box-sizing:border-box;"></div>';
    html += '<div class="sm gray mt-8">\u4e0b\u62c9\u9009\u4e66 \u2192 \u70b9\u300c\u6fc0\u6d3b\u5f53\u524d\u4e66\u300d \u2192 \u4e0b\u65b9\u81ea\u52a8\u663e\u793a\u8be5\u4e66\u7684\u62d8\u53e0\u6761\u76ee\u5217\u8868\u3002\u5982\u6709\u591a\u672c\u6fc0\u6d3b\uff0c\u6bcf\u672c\u5404\u81ea\u62d8\u53e0\u3002</div></div>';

    // ===== \u6761\u76ee\u7ea7\u9009\u62e9（\u6bcf\u672c\u5df2\u6fc0\u6d3b\u4e66\u5404\u81ea\u62d8\u53e0\u628a\u5408\u53e0 =====
    html += '<div class="card" id="htyq-wb-entries-card"><div class="card-title">\ud83d\udcd1 \u6761\u76ee\u7ea7\u9009\u62e9 <span class="bdg">\u5df2\u6fc0\u6d3b\u7684\u4e66\u5404\u81ea\u53ef\u62d8\u53e0</span></div>';
    html += '<div id="htyq-wb-entries-container">';
    html += '<div class="sm gray" style="padding:10px 0;" id="htyq-wb-entries-empty">\u6682\u65e0\u5df2\u6fc0\u6d3b\u4e16\u754c\u4e66\uff0c\u8bf7\u5148\u5728\u4e0a\u65b9\u6fc0\u6d3b</div>';
    html += '</div>';
    html += '<div class="flex mt-8" style="gap:4px;">';
    html += '<button class="btn btn-primary" id="htyq-save-wb-entries">\ud83d\udcbe \u4fdd\u5b58\u5168\u90e8\u6761\u76ee\u9009\u62e9</button>';
    html += '<button class="btn btn-sm" id="htyq-refresh-entries">\ud83d\udd04 \u5237\u65b0\u6761\u76ee</button>';
    html += '</div></div>';

    // ===== \u7edf\u8ba1 =====
    html += '<div class="card"><div class="card-title">\ud83d\udccb \u6d3b\u52a8\u6761\u76ee\u7edf\u8ba1</div>';
    html += '<div class="flex"><span class="gray">\u603b\u4e16\u754c\u4e66\uff1a</span><span id="htyq-wb-stats-books">0 \u672c</span></div>';
    html += '<div class="flex"><span class="gray">\u5df2\u6fc0\u6d3b\u4e66\uff1a</span><span id="htyq-wb-stats-active-books">0 \u672c</span></div>';
    html += '<div class="flex"><span class="gray">\u6709\u6761\u76ee\u914d\u7f6e\u7684\u4e66\uff1a</span><span id="htyq-wb-stats-entry-books">0 \u672c</span></div>';
    html += '<div class="flex mt-4" style="align-items:center;gap:6px;"><label style="font-size:11px;color:#8b949e;cursor:pointer;"><input type="checkbox" id="htyq-wb-auto-activate"'+(_aa?' checked':'')+' style="accent-color:#238636;"> \u5207\u6362\u4e16\u754c\u4e66/\u573a\u666f\u65f6\u81ea\u52a8\u6fc0\u6d3b</label></div>';
    html += '<div class="fa mt-8"><button class="btn btn-sm" id="htyq-wb-analyze">\ud83e\udde0 AI \u5206\u6790\u4e16\u754c\u4e66\u5173\u8054\u6027</button></div></div>';
    ...
    function updateStats() {
      try {
        ...
        var selectEl = document.getElementById('htyq-wb-select');
        var totalBooks = selectEl ? selectEl.options.length - 1 : 0;
        var elB = document.getElementById('htyq-wb-stats-books');
        var elAB = document.getElementById('htyq-wb-stats-active-books');
        var elEB = document.getElementById('htyq-wb-stats-entry-books');
        var elCnt = document.getElementById('htyq-wb-selected-count');
    ...
    async function loadWbDropdown() {
      ...
      try {
        ...
        updateStats();
      } catch(e) { console.warn('[HTYQ UI] \u52a0\u8f7d\u4e16\u754c\u4e66\u4e0b\u62c9\u5931\u8d25', e); }
    }
    ...
    async function renderAllBookEntries() {
      var container = document.getElementById('htyq-wb-entries-container');
      var emptyEl = document.getElementById('htyq-wb-entries-empty');
      if (!container) return;
      var bookSel = (typeof worldbook.getBookSelection === 'function') ? worldbook.getBookSelection() : {};
      var activatedBooks = [];
      for (var bk in bookSel) { if (bookSel[bk]) activatedBooks.push(bk); }
      if (activatedBooks.length === 0) {
        container.innerHTML = '<div class="sm gray" style="padding:10px 0;" id="htyq-wb-entries-empty">\u6682\u65e0\u5df2\u6fc0\u6d3b\u4e16\u754c\u4e66\uff0c\u8bf7\u5148\u5728\u4e0a\u65b9\u6fc0\u6d3b</div>';
        return;
      }
      // \u663e\u793a\u52a0\u8f7d\u4e2d
    ...
    function updateDropdownText(bookName, activated) {
      var select = document.getElementById('htyq-wb-select');
      if (!select) return;
    ...
    setTimeout(function() {
      // \u641c\u7d22 = \u8fc7\u6ee4\u4e0b\u62c9\u6846\u9009\u9879
      var searchEl = document.getElementById('htyq-wb-search');
      if (searchEl) {
      ...
      var deactSel = document.getElementById('htyq-wb-deactivate-selected');
      if (deactSel) {
      ...
      var actAll = document.getElementById('htyq-wb-activate-all');
      if (actAll) {
      ...
      var deactAll = document.getElementById('htyq-wb-deactivate-all');
      if (deactAll) {
      ...
      var saveWb = document.getElementById('htyq-save-wb');
      if (saveWb) {
      ...
      // \u5237\u65b0\u5217\u8868
      var refreshBtn = document.getElementById('htyq-refresh-wb');
      if (refreshBtn) {
      ...
      // \u5237\u65b0\u6761\u76ee\u6309\u94ae
      var refreshEntries = document.getElementById('htyq-refresh-entries');
      if (refreshEntries) {
      ...
      var autoActCb = document.getElementById('htyq-wb-auto-activate');
      if (autoActCb) {
        autoActCb.addEventListener('change', function() {
          localStorage.setItem('htyq_wb_autoActivate', this.checked ? 'true' : 'false');
        });
      }
      // AI analyze worldbook button
      var wbAnalyzeBtn = document.getElementById('htyq-wb-analyze');
      if (wbAnalyzeBtn) {
  ...
  function renderSettings(cont) {
    ...
    html += '<div class="fg"><label>\u70ed\u8bb0\u5fc6\u9608\u503c\uff08\u8f6e\u6570\uff09</label><input type="number" id="htyq-hot-threshold" value="50"></div>';
    ...
    html += '<div class="fg"><label>\u5f53\u524d\u9884\u8bbe</label><select id="htyq-preset-select"><option selected>'+esc(currentPreset)+'</option></select></div>';
    html += '<div class="fg"><label>\u521b\u5efa\u65b0\u9884\u8bbe</label><div class="flex"><input type="text" id="htyq-new-preset-name" placeholder="\u8f93\u5165\u9884\u8bbe\u540d\u79f0..." style="flex:1;padding:6px 10px;background:#0d1117;border:1px solid #30363d;border-radius:6px;color:#e6edf3;font-size:12px;"><button class="btn btn-sm" id="htyq-create-preset">\u2795 \u521b\u5efa</button></div></div></div>';
    html += '<div class="fa"><button class="btn btn-primary" id="htyq-save-preset">\ud83d\udcbe \u4fdd\u5b58\u9884\u8bbe</button><button class="btn btn-sm" id="htyq-export-preset">\ud83d\udce4 \u5bfc\u51fa\u9884\u8bbe</button><button class="btn btn-sm" id="htyq-import-preset">\ud83d\udce5 \u5bfc\u5165\u9884\u8bbe</button><button class="btn btn-danger btn-sm" id="htyq-delete-preset">\ud83d\uddd1\ufe0f \u5220\u9664\u9884\u8bbe</button></div></div>';
    ...
    DIM_DEFS.forEach(function(dim){
      ...
      html += '<div class="fg"><label>'+dim.label+'</label><select class="htyq-wl-dim" data-dim="'+dim.id+'">';
    ...
    html += '</div></div>';
    html += '<div class="flex" style="gap:4px;"><input type="text" id="htyq-add-wl-rule" placeholder="\u8f93\u5165\u81ea\u5b9a\u4e49\u6cd5\u5219..." style="flex:1;padding:4px 8px;background:#0d1117;border:1px solid #30363d;border-radius:4px;color:#e6edf3;font-size:11px;"><button class="btn btn-sm" id="htyq-add-wl-rule-btn">\u2795 \u6dfb\u52a0</button></div>';
    ...
    html += '\u7cfb\u7edf\u5c1a\u672a\u5bf9\u5f53\u524d\u4e16\u754c\u8fdb\u884c AI \u5206\u6790\u3002<button class="btn btn-sm" id="htyq-analyze-wl">\u5f00\u59cb\u5206\u6790</button></div></div>';
    html += '<div class="fa"><button class="btn btn-primary" id="htyq-save-wl">\ud83d\udcbe \u4fdd\u5b58\u4e16\u754c\u6cd5\u5219</button></div>';
    // ★ v3.0.1: 数据导出
    html += '<div class="card"><div class="card-title">\ud83d\udce6 \u6570\u636e\u5bfc\u51fa <span class="bdg">\u5907\u4efd/\u8fc1\u79fb</span></div>';
    html += '<div class="sm gray" style="margin-bottom:8px;">\u5bfc\u51fa\u5168\u90e8 HTYQ \u72b6\u6001\u6570\u636e\u4e3a JSON \u6587\u4ef6\uff0c\u53ef\u7528\u4e8e\u5907\u4efd\u6216\u8de8\u804a\u5929\u8fc1\u79fb\u3002</div>';
    html += '<div class="fa"><button class="btn btn-primary" id="htyq-export-all">\ud83d\udce4 \u5bfc\u51fa\u5168\u90e8\u6570\u636e (JSON)</button>';
    html += '<button class="btn" id="htyq-import-all" style="margin-left:8px;">\ud83d\udce5 \u5bfc\u5165\u6570\u636e</button>';
    html += '<input type="file" id="htyq-import-file" accept=".json" style="display:none;"></div></div>';
    ...
    // bind events
    setTimeout(function(){
      var saveAll = document.getElementById('htyq-save-all');
      if (saveAll) saveAll.addEventListener('click', function(){
        ...
        ['save-api','save-general','save-storage','save-preset','save-wl','save-drive','save-time','save-evolve','save-inject'].forEach(function(id){
          var btn = document.getElementById('htyq-' + id);
          if (btn) btn.click();
        });
        toast('\u2705 \u5168\u90e8\u8bbe\u7f6e\u5df2\u4fdd\u5b58');
      });

      var saveApi = document.getElementById('htyq-save-api');
      ...
      if (exportSnap) exportSnap.addEventListener('click', function(){
        ...
        a.download = 'htyq-snapshot-'+new Date().toISOString().slice(0,19).replace(/[:-]/g,'')+'.json';
        a.click();
        toast('\ud83d\udce6 \u5feb\u7167\u5df2\u5bfc\u51fa');
      });

      // \u5bfc\u5165\u5feb\u7167
      var importSnap = document.getElementById('htyq-import-snapshot');
      ...
      // \u9884\u8bbe\u7ba1\u7406\u6309\u94ae
      var savePreset = document.getElementById('htyq-save-preset');
      if (savePreset) savePreset.addEventListener('click', function(){
        var s = JSON.parse(localStorage.getItem('htyq_lite_settings')||'{}');
        s.activePreset = document.getElementById('htyq-preset-select') ? document.getElementById('htyq-preset-select').value : '\u6807\u51c6\u9884\u8bbe';
        localStorage.setItem('htyq_lite_settings', JSON.stringify(s));
        toast('\u2705 \u9884\u8bbe\u5df2\u4fdd\u5b58');
      });
      var createPreset = document.getElementById('htyq-create-preset');
      if (createPreset) createPreset.addEventListener('click', function(){
        var name = document.getElementById('htyq-new-preset-name');
        if (name && name.value.trim()) {
          var sel = document.getElementById('htyq-preset-select');
      ...
      var exportPreset = document.getElementById('htyq-export-preset');
      if (exportPreset) exportPreset.addEventListener('click', function(){
        var s = JSON.parse(localStorage.getItem('htyq_lite_settings')||'{}');
        var blob = new Blob([JSON.stringify(s, null, 2)], {type:'application/json'});
        var a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
        a.download = 'htyq-preset-'+new Date().toISOString().slice(0,19).replace(/[:-]/g,'')+'.json';
        a.click();
        toast('\ud83d\udce4 \u9884\u8bbe\u5df2\u5bfc\u51fa');
      });
      var importPreset = document.getElementById('htyq-import-preset');
      if (importPreset) importPreset.addEventListener('click', function(){
        ...
        inp.addEventListener('change', function(e){
          ...
          reader.onload = function(ev){
            try {
              var data = JSON.parse(ev.target.result);
              localStorage.setItem('htyq_lite_settings', JSON.stringify(data));
              toast('\u2705 \u9884\u8bbe\u5df2\u5bfc\u5165');
            } catch(err){ toast('\u274c \u5bfc\u5165\u5931\u8d25\uff1a' + err.message, true); }
          };
          reader.readAsText(file);
      ...
      var deletePreset = document.getElementById('htyq-delete-preset');
      if (deletePreset) deletePreset.addEventListener('click', function(){
        if (!confirm('\u786e\u5b9a\u5220\u9664\u5f53\u524d\u9884\u8bbe\uff1f\u6b64\u64cd\u4f5c\u4e0d\u53ef\u6062\u590d\u3002')) return;
        toast('\u2705 \u9884\u8bbe\u5df2\u5220\u9664');
      });

      var saveWl = document.getElementById('htyq-save-wl');
      ...
      // \u4e16\u754c\u6cd5\u5219 AI \u5206\u6790
      var awl = document.getElementById('htyq-analyze-wl');
      ...
      var addRuleBtn = document.getElementById('htyq-add-wl-rule-btn');
      if (addRuleBtn) addRuleBtn.addEventListener('click', function(){
        var input = document.getElementById('htyq-add-wl-rule');
      ...
      if (resetAll) resetAll.addEventListener('click', function(){
        if (!confirm('\u786e\u5b9a\u91cd\u7f6e\u6240\u6709 HTYQ \u6570\u636e\uff1f\u8fd9\u5c06\u6e05\u9664\u5168\u90e8\u8bb0\u5fc6\u3001\u6210\u5c31\u3001\u4e16\u754c\u72b6\u6001\u3001\u60c5\u611f\u6570\u636e\u3002')) return;
        if (!confirm('\u518d\u786e\u8ba4\u4e00\u6b21\uff1a\u8fd9\u4e2a\u64cd\u4f5c\u4e0d\u53ef\u6062\u590d\u3002\u786e\u5b9a\u8981\u91cd\u7f6e\uff1f')) return;
        localStorage.removeItem('htyq_lite_state');
        toast('\u26a0\ufe0f \u6240\u6709 HTYQ \u6570\u636e\u5df2\u91cd\u7f6e\uff0c\u8bf7\u5237\u65b0\u9875\u9762');
        refresh();
      });
      // reset settings only
      var resetS = document.getElementById('htyq-reset-settings');
  ...
  function renderHelp(cont, state) {
    ...
    html += '<input type="text" class="htyq-search-input" id="htyq-help-search" placeholder="搜索功能/关键词..." style="max-width:200px;"></div>';
    html += '<div class="save-bar"><div class="hint">🔍 搜索帮助内容</div>';
    // \u65b0\u624b/\u8fdb\u9636 Tab \u5207\u6362
    html += '<div style="display:flex;gap:0;margin-bottom:14px;border-bottom:2px solid #21262d;">';
    html += '<button class="htyq-help-tab-btn active" data-htab="newbie" style="background:none;border:none;color:#f0c040;font-size:13px;font-weight:600;padding:9px 20px;border-bottom:2px solid #f0c040;cursor:pointer;font-family:inherit;">\ud83d\udfe2 \u65b0\u624b\u5165\u95e8</button>';
    html += '<button class="htyq-help-tab-btn" data-htab="advanced" style="background:none;border:none;color:#8b949e;font-size:13px;padding:9px 20px;border-bottom:2px solid transparent;cursor:pointer;font-family:inherit;">\ud83d\udd35 \u8fdb\u9636\u53c2\u8003</button></div>';

    // ========== \u65b0\u624b\u5165\u95e8 ==========
    html += '<div class="htyq-help-panel" id="htyq-help-newbie">';
    ...
    // ========== \u8fdb\u9636\u53c2\u8003 ==========
    html += '<div class="htyq-help-panel" id="htyq-help-advanced" style="display:none;">';
    ...
    setTimeout(function(){
      document.querySelectorAll('.htyq-help-tab-btn').forEach(function(btn){
        btn.addEventListener('click', function(){
          document.querySelectorAll('.htyq-help-tab-btn').forEach(function(b){
            b.style.color = '#8b949e';
            b.style.borderBottomColor = 'transparent';
            b.style.fontWeight = '400';
          });
          document.querySelectorAll('.htyq-help-panel').forEach(function(p){ p.style.display = 'none'; });
