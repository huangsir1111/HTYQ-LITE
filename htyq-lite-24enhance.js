// htyq-lite-24enhance.js — 24项UI增强模块（独立自包含）
// ============================================================
// 与现有插件完全兼容，不修改任何现有文件
...
(function() {
  'use strict';
  if (window.__HTYQ_24_ENHANCED__) return;
  window.__HTYQ_24_ENHANCED__ = true;

  var core = window.HTYQ_LITE_CORE;
  if (!core) { console.warn('[24enh] HTYQ_LITE_CORE not found'); return; }
  ...
  function showEvolveProgress() {
    ...
    overlay.className = 'htyq-evolve-progress-overlay';
    overlay.id = 'htyq-evolve-progress';
    overlay.innerHTML =
      '<div class="htyq-evolve-progress-box">' +
        '<div class="htyq-evolve-progress-title">⏳ 世界推演中...</div>' +
        '<div class="htyq-evolve-progress-bar">' +
          '<div class="htyq-evolve-progress-fill" id="htyq-evolve-fill"></div>' +
        '</div>' +
        '<div class="htyq-evolve-step-label" id="htyq-evolve-step">初始化...</div>' +
      '</div>';
    ...
    var stepIdx = 0;
    var fill = document.getElementById('htyq-evolve-fill');
    var stepEl = document.getElementById('htyq-evolve-step');
  ...
  function removeEvolveProgress() {
    var existing = document.getElementById('htyq-evolve-progress');
    if (existing) {
  ...
  function savePanelState() {
    var panel = document.querySelector('.htyq-lite-panel');
    if (!panel) return;
    var state = {
      ...
      tab: getActiveTab(),
      autoHide: window._htyq_autoHideEnabled || false
    };
    try { localStorage.setItem('htyq_panel_state', JSON.stringify(state)); } catch(e) {}
  }

  function loadPanelState() {
    try {
      var raw = localStorage.getItem('htyq_panel_state');
      if (!raw) return;
      ...
      if (state.autoHide) {
        window._htyq_autoHideEnabled = true;
        setupAutoHide();
      }
    } catch(e) {}
  }
  ...
  function renderLastEvolveCard(container, state) {
    ...
    var html =
      '<div class="htyq-last-evolve-card card">' +
        '<div class="card-title">📋 上次演化结果</div>' +
        '<div class="htyq-last-evolve-item">🔄 轮次 #' + (result.round || '?') + '</div>' +
        '<div class="htyq-last-evolve-item">📝 记忆数: ' + (result.newMemories || 0) + '</div>' +
        '<div class="htyq-last-evolve-item">📌 事件数: ' + (result.newEvents || 0) + '</div>' +
      '</div>';
  ...
  function renderUndoList(container, state) {
    ...
    if (!history.length) {
      ...
      for (var i = history.length - 1; i >= 0; i--) {
        var h = history[i];
        html +=
          '<div class="htyq-undo-item" style="display:flex;justify-content:space-between;align-items:center;padding:4px 0;border-bottom:1px solid #21262d;">' +
            ...
            '<button class="btn btn-sm htyq-undo-rollback" data-idx="' + h.index + '" style="color:#d29922;">↩️ 回滚</button>' +
          '</div>';
      }
    }
    html += '</div>';
    ...
    // 绑定回滚按钮
    setTimeout(function() {
      container.querySelectorAll('.htyq-undo-rollback').forEach(function(btn) {
  ...
  function showAchievementPopup(state, achId, achData) {
    ...
    popup.className = 'htyq-ach-popup-overlay';
    popup.id = 'htyq-ach-popup-' + Date.now();
    popup.innerHTML =
      '<div class="htyq-ach-popup">' +
        '<div class="htyq-ach-popup-inner">' +
          '<div class="htyq-ach-popup-icon">' + icon + '</div>' +
          '<div class="htyq-ach-popup-name">' + escHtml(name) + '</div>' +
          '<div class="htyq-ach-popup-desc">' + escHtml(desc) + '</div>' +
          '<div class="htyq-ach-popup-rarity ' + rarity + '">' + rarityStars + '</div>' +
        '</div>' +
        '<div class="htyq-confetti">' +
          Array(20).fill(0).map(function(){ return '<span class="htyq-confetti-particle"></span>'; }).join('') +
        '</div>' +
      '</div>';
  ...
  function renderDonutChart(container, state) {
    ...
    var html =
      '<div class="htyq-donut-container card">' +
        '<div class="card-title">🎯 总完成率</div>' +
        '<div style="display:flex;align-items:center;gap:16px;">' +
          '<svg width="130" height="130" viewBox="0 0 130 130" class="htyq-donut-svg">' +
  ...
  function renderAchTypeTabs(container, state) {
    ...
    var html =
      ...
      '<div class="htyq-ach-type-tabs">';
    types.forEach(function(t) {
      var active = (state.achTypeFilter === t.id || (!state.achTypeFilter && t.id === 'all')) ? ' active' : '';
      html += '<button class="htyq-ach-type-tab' + active + '" data-type="' + t.id + '">' + t.label + '</button>';
    });
    html += '</div></div>';
    container.insertAdjacentHTML('afterbegin', html);
    ...
    // 绑定点击
    setTimeout(function() {
      container.querySelectorAll('.htyq-ach-type-tab').forEach(function(tab) {
        tab.addEventListener('click', function() {
          container.querySelectorAll('.htyq-ach-type-tab').forEach(function(t) { t.classList.remove('active'); });
  ...
  function renderBondCanvas(container, state) {
    ...
    wrap.className = 'htyq-bond-canvas-wrap card';
    wrap.innerHTML = '<div class="card-title">🔗 关系图</div><canvas class="htyq-bond-canvas" width="400" height="300"></canvas>';
    ...
    canvas.addEventListener('mousemove', function(e) {
      ...
      var tooltip = wrap.querySelector('.htyq-bond-tooltip');
      if (found) {
        if (!tooltip) {
          tooltip = document.createElement('div');
          tooltip.className = 'htyq-bond-tooltip';
  ...
  function renderWeatherPanel(container, state) {
    ...
    var html =
      '<div class="htyq-weather-panel card">' +
        '<div class="card-title">🌤️ 天气与季节</div>' +
        '<div style="display:flex;align-items:center;gap:12px;margin-bottom:8px;">' +
          '<div class="htyq-weather-icon" style="font-size:36px;">' + (weather.split(' ')[0] || '🌤️') + '</div>' +
          '<div class="htyq-weather-info">' +
            '<div style="font-size:16px;font-weight:600;">' + escHtml(weather) + '</div>' +
            '<div style="font-size:13px;color:#8b949e;">' + season + '季</div>' +
          '</div>' +
        '</div>' +
        '<div class="htyq-weather-controls" style="display:flex;gap:8px;flex-wrap:wrap;">' +
          '<div class="fg" style="flex:1;min-width:120px;"><label>天气</label>' +
            '<select id="htyq-weather-select">' + weatherOpts.map(function(w) { return '<option' + (w === weather ? ' selected' : '') + '>' + w + '</option>'; }).join('') + '</select></div>' +
          '<div class="fg" style="flex:0 0 80px;"><label>季节</label>' +
            '<select id="htyq-season-select">' + seasonOpts.map(function(s) { return '<option' + (s === season ? ' selected' : '') + '>' + s + '</option>'; }).join('') + '</select></div>' +
          '<div class="fa" style="border:none;padding:0;margin:0;align-self:flex-end;">' +
            '<button class="btn btn-primary btn-sm" id="htyq-weather-save">💾 保存</button></div>' +
        '</div>' +
      '</div>';
    container.appendChild(html);

    setTimeout(function() {
      var saveBtn = document.getElementById('htyq-weather-save');
      if (saveBtn) {
        saveBtn.addEventListener('click', function() {
          var st = core.loadState();
          var ws = document.getElementById('htyq-weather-select');
          var ss = document.getElementById('htyq-season-select');
  ...
  function renderMemPagination(container, state) {
    ...
    var html =
      '<div class="htyq-mem-pagination">' +
      '<button class="btn btn-sm htyq-mem-page-btn" data-page="prev" ' + (_memPageCurrent <= 0 ? 'disabled' : '') + '>◀ 上一页</button>' +
      '<span class="htyq-mem-page-info" id="htyq-page-info">第 ' + (_memPageCurrent + 1) + '/' + totalPages + ' 页（共 ' + memories.length + ' 条）</span>' +
      '<button class="btn btn-sm htyq-mem-page-btn" data-page="next" ' + (_memPageCurrent >= totalPages - 1 ? 'disabled' : '') + '>下一页 ▶</button>' +
      '</div>';
    container.appendChild(html);

    setTimeout(function() {
      container.querySelectorAll('.htyq-mem-page-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
          ...
          var st = core.loadState();
          applyMemPageFilter();
          var infoEl = document.getElementById('htyq-page-info');
          if (infoEl) {
          ...
          container.querySelectorAll('.htyq-mem-page-btn').forEach(function(b) {
...
// ===== Feature 11: 记忆回收站 =====
  function renderRecycleBin(container, state) {
    ...
    var html =
      '<div class="card" id="htyq-recycle-section">' +
        '<div class="card-title">🗑️ 记忆回收站 <span class="bdg">' + bin.length + ' 条</span>' +
          '<button class="btn btn-sm htyq-recycle-toggle" style="margin-left:auto;color:#8b949e;" id="htyq-recycle-toggle">' +
            (window._htyq_showRecycle ? '🔼 收起' : '🗑️ 打开回收站') +
          '</button>' +
        '</div>';
    if (window._htyq_showRecycle) {
      if (!bin.length) {
        ...
        for (var i = 0; i < bin.length; i++) {
          var mem = bin[i];
          html +=
            '<div class="htyq-recycle-item" style="display:flex;justify-content:space-between;align-items:center;padding:4px 0;border-bottom:1px solid #21262d;">' +
              ...
              '<div style="display:flex;gap:4px;flex-shrink:0;">' +
                '<button class="btn btn-sm htyq-recycle-restore" data-idx="' + i + '" style="color:#7ee787;">↩️ 恢复</button>' +
                '<button class="btn btn-sm htyq-recycle-perma" data-idx="' + i + '" style="color:#f85149;">🗑️ 删除</button>' +
              '</div>' +
            '</div>';
        }
        html += '<div style="margin-top:6px;"><button class="btn btn-sm btn-danger" id="htyq-recycle-empty">🗑️ 清空回收站</button></div>';
      }
    }
    html += '</div>';
    container.appendChild(html);
    ...
    setTimeout(function() {
      var toggle = document.getElementById('htyq-recycle-toggle');
      if (toggle) toggle.addEventListener('click', function() {
        window._htyq_showRecycle = !window._htyq_showRecycle;
        refreshCurrentTab();
      });
      container.querySelectorAll('.htyq-recycle-restore').forEach(function(btn) {
        btn.addEventListener('click', function() {
          var idx = parseInt(this.getAttribute('data-idx'));
          var st = core.loadState();
      ...
      container.querySelectorAll('.htyq-recycle-perma').forEach(function(btn) {
        btn.addEventListener('click', function() {
          var idx = parseInt(this.getAttribute('data-idx'));
          var st = core.loadState();
      ...
      var emptyBtn = document.getElementById('htyq-recycle-empty');
  ...
  function renderMemAssocCanvas(container, state) {
    ...
    var wrap = document.createElement('div');
    wrap.className = 'htyq-mem-assoc-wrap card';
    wrap.innerHTML = '<div class="card-title">🔗 记忆关联网络</div><canvas class="htyq-mem-assoc-canvas" width="400" height="250"></canvas>';
    container.appendChild(wrap);
    ...
    canvas.addEventListener('mousemove', function(e) {
      ...
      if (found) {
        if (!tooltip) { tooltip = document.createElement('div'); tooltip.className = 'htyq-mem-assoc-tooltip'; wrap.appendChild(tooltip); }
    ...
    var html =
      ...
      '<div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">' +
        '<div class="fg" style="flex:1;min-width:120px;"><label>推演间隔</label>' +
          '<select id="htyq-schedule-interval">' +
            options.map(function(o) { return '<option value="' + o.v + '"' + (schedule.interval === o.v ? ' selected' : '') + '>' + o.l + '</option>'; }).join('') +
          '</select></div>' +
        '<div class="htyq-schedule-status ' + (isRunning ? 'running' : 'stopped') + '" id="htyq-schedule-status">' +
          (isRunning ? '🟢 运行中' : '⚪ 已停止') +
        '</div>' +
      '</div>' +
      '<div class="fa"><button class="btn btn-primary" id="htyq-schedule-start">' + (isRunning ? '🔄 重启' : '▶️ 启动') + '</button>' +
      '<button class="btn btn-sm" id="htyq-schedule-stop"' + (!isRunning ? ' disabled' : '') + '>⏹️ 停止</button></div>' +
      '</div>';
    container.appendChild(html);

    setTimeout(function() {
      var startBtn = document.getElementById('htyq-schedule-start');
      var stopBtn = document.getElementById('htyq-schedule-stop');
  ...
  function startScheduleEvolve() {
    stopScheduleEvolve();
    var sel = document.getElementById('htyq-schedule-interval');
    ...
    var statusEl = document.getElementById('htyq-schedule-status');
    if (statusEl) {
      statusEl.className = 'htyq-schedule-status running';
  ...
  function stopScheduleEvolve() {
    ...
    var statusEl = document.getElementById('htyq-schedule-status');
    if (statusEl) {
      statusEl.className = 'htyq-schedule-status stopped';
  ...
  function renderPreviewModal(previewData, onConfirm, onCancel) {
    var overlay = document.createElement('div');
    overlay.className = 'htyq-preview-modal';
    overlay.id = 'htyq-preview-overlay';
    var html =
      '<div class="card" style="max-width:500px;">' +
        '<div class="card-title">👁️ 推演预览</div>' +
        '<div class="htyq-preview-content">';
    if (previewData && previewData.length) {
      previewData.forEach(function(item) {
        html += '<div class="htyq-preview-item">' +
          '<span class="htyq-evolve-tag" style="background:' + (item.type === 'add' ? '#1a3a1a' : item.type === 'change' ? '#3a2a1a' : '#3a1a1a') + ';color:' + (item.type === 'add' ? '#7ee787' : item.type === 'change' ? '#d29922' : '#f85149') + ';padding:1px 6px;border-radius:4px;font-size:10px;margin-right:6px;">' + item.label + '</span>' +
          escHtml(item.text) +
        '</div>';
      });
    } else {
    ...
    html += '</div><div class="htyq-preview-actions" style="margin-top:10px;display:flex;gap:8px;justify-content:flex-end;">' +
      '<button class="btn btn-sm" id="htyq-preview-cancel">取消</button>' +
      '<button class="btn btn-primary" id="htyq-preview-confirm">✅ 确认执行</button>' +
      '</div></div>';
    overlay.innerHTML = html;
    document.body.appendChild(overlay);

    document.getElementById('htyq-preview-confirm').addEventListener('click', function() {
      if (overlay.parentNode) overlay.parentNode.removeChild(overlay);
      if (typeof onConfirm === 'function') onConfirm();
    });
    document.getElementById('htyq-preview-cancel').addEventListener('click', function() {
      if (overlay.parentNode) overlay.parentNode.removeChild(overlay);
  ...
  function setupApiMonitor() {
    ...
    monitor.className = 'htyq-api-monitor';
    monitor.id = 'htyq-api-monitor';
    monitor.style.marginLeft = 'auto';
    monitor.innerHTML =
      '<span class="htyq-api-monitor-dot yellow" id="htyq-api-dot"></span>' +
      '<span class="htyq-api-monitor-text" id="htyq-api-monitor-text" style="font-size:10px;color:#8b949e;margin-left:4px;">检测中...</span>';
    ...
    monitor.addEventListener('click', function() {
      var dot = document.getElementById('htyq-api-dot');
      var text = document.getElementById('htyq-api-monitor-text');
      if (dot) dot.className = 'htyq-api-monitor-dot yellow';
  ...
  function pingApi() {
    var textEl = document.getElementById('htyq-api-monitor-text');
    var dot = document.getElementById('htyq-api-dot');
    var latencyEl = document.getElementById('htyq-api-latency');
    ...
    try {
      ...
      xhr.onload = function() {
        ...
        if (dot) { dot.className = 'htyq-api-monitor-dot green'; dot.title = latency + 'ms'; }
        if (textEl) textEl.textContent = '在线';
        updateApiLatency(latency);
      };
      xhr.onerror = function() {
        if (dot) dot.className = 'htyq-api-monitor-dot red';
  ...
  function updateApiLatency(latency) {
    var existing = document.getElementById('htyq-api-latency');
    if (latency !== null) {
      if (!existing) {
        var textEl = document.getElementById('htyq-api-monitor-text');
        if (textEl) {
          var span = document.createElement('span');
          span.id = 'htyq-api-latency';
  ...
  function renderDeepProfile(container, state, charName) {
    ...
    var html =
      '<div class="htyq-deep-profile-card card">' +
        '<div class="htyq-deep-profile-header" style="display:flex;align-items:center;gap:12px;">' +
          '<div class="htyq-deep-profile-avatar" style="font-size:40px;">' + (portrait.emoji || '👤') + '</div>' +
        ...
        '</div>' +
        '<div class="htyq-deep-profile-stats" style="display:flex;gap:8px;flex-wrap:wrap;margin:8px 0;">' +
    ...
    // 关键事件
    if (events.length) {
      html += '<div class="htyq-deep-profile-events" style="margin:6px 0;"><div style="font-size:11px;color:#f0c040;margin-bottom:4px;">📜 关键事件</div>';
    ...
    // 关系
    if (relations.length) {
      html += '<div class="htyq-deep-profile-relations"><div style="font-size:11px;color:#f0c040;margin-bottom:4px;">🤝 关系</div>';
  ...
  function renderChapterManager(container, state) {
    ...
    var html =
      '<div class="htyq-chapter-manager card">' +
    ...
    if (chapters.length === 0) {
      ...
      chapters.forEach(function(ch, i) {
        ...
        html += '<div class="htyq-chapter-item' + (isActive ? ' editing' : '') + '" style="display:flex;justify-content:space-between;align-items:center;padding:6px 0;border-bottom:1px solid #21262d;">' +
          '<div style="flex:1;">' +
          ...
          '<div class="ch-actions" style="display:flex;gap:4px;flex-shrink:0;">' +
            (!isActive ? '<button class="btn btn-sm htyq-chapter-activate" data-idx="' + i + '" style="color:#7ee787;">📌 激活</button>' : '<span style="font-size:10px;color:#f0c040;">✅ 当前</span>') +
            '<button class="btn btn-sm htyq-chapter-rename" data-idx="' + i + '" style="color:#58a6ff;">✏️</button>' +
            '<button class="btn btn-sm htyq-chapter-delete" data-idx="' + i + '" style="color:#f85149;">🗑️</button>' +
          '</div>' +
        '</div>';
      });
    }
    ...
    html += '<div class="fa" style="border:none;padding:0;margin-top:8px;">' +
      '<button class="btn btn-primary btn-sm" id="htyq-chapter-create">➕ 新建章节</button>' +
      '<button class="btn btn-sm" id="htyq-chapter-auto">📖 自动生成章节</button></div>' +
      '</div>';
    container.appendChild(html);

    setTimeout(function() {
      document.getElementById('htyq-chapter-create') && document.getElementById('htyq-chapter-create').addEventListener('click', function() {
      ...
      container.querySelectorAll('.htyq-chapter-activate').forEach(function(btn) {
        btn.addEventListener('click', function() {
          var idx = parseInt(this.getAttribute('data-idx'));
          var st = core.loadState();
      ...
      container.querySelectorAll('.htyq-chapter-rename').forEach(function(btn) {
        btn.addEventListener('click', function() {
          var idx = parseInt(this.getAttribute('data-idx'));
          var st = core.loadState();
      ...
      container.querySelectorAll('.htyq-chapter-delete').forEach(function(btn) {
        btn.addEventListener('click', function() {
          var idx = parseInt(this.getAttribute('data-idx'));
      ...
      document.getElementById('htyq-chapter-auto') && document.getElementById('htyq-chapter-auto').addEventListener('click', function() {
        var st = core.loadState();
        if (!st.chapters) st.chapters = [];
  ...
  function renderCombatChart(container, state) {
    ...
    var html =
      ...
      '<div class="htyq-combat-summary-strip" style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:8px;">' +
      ...
      '</div>' +
      '<div class="htyq-combat-chart-wrap" style="display:flex;gap:4px;align-items:flex-end;height:80px;padding:4px 0;">';
    ...
    rounds.forEach(function(r) {
      ...
      html += '<div style="flex:1;display:flex;flex-direction:column;align-items:center;height:100%;justify-content:flex-end;">' +
        '<div class="htyq-combat-chart-bar" style="height:' + pct + '%;width:80%;background:linear-gradient(180deg,#f85149,#b62324);border-radius:3px 3px 0 0;min-height:2px;"></div>' +
        '<div class="htyq-combat-chart-label" style="font-size:8px;color:#8b949e;margin-top:2px;">#' + r + '</div>' +
      '</div>';
    });
  ...
  function showEntryPreview(entry) {
    if (!entry) return;
    var overlay = document.createElement('div');
    overlay.className = 'htyq-entry-preview-overlay';
    overlay.innerHTML =
      '<div class="htyq-entry-preview-modal card" style="max-width:550px;">' +
        '<div class="htyq-entry-preview-header" style="display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #21262d;padding-bottom:8px;margin-bottom:8px;">' +
          ...
          '<button class="btn btn-sm" id="htyq-close-preview" style="color:#f85149;">✕ 关闭</button>' +
        '</div>' +
        '<div class="htyq-entry-preview-body" style="font-size:12px;color:#e6edf3;line-height:1.7;max-height:300px;overflow-y:auto;white-space:pre-wrap;">' +
          escHtml(entry.content || entry.text || entry.description || '(无内容)') +
        '</div>' +
        '<div class="htyq-entry-preview-tags" style="margin-top:8px;padding-top:8px;border-top:1px solid #21262d;">' +
          '<span style="font-size:10px;color:#8b949e;">索引: ' + (entry.index || entry.id || '#') + '</span>' +
        '</div>' +
      '</div>';
    document.body.appendChild(overlay);
    overlay.addEventListener('click', function(e) {
      if (e.target === overlay || e.target.id === 'htyq-close-preview') {
        overlay.parentNode.removeChild(overlay);
      }
    });
  }
  ...
  function renderTriggerTest(container, state) {
    var html =
      ...
      '<button class="btn btn-primary btn-sm" id="htyq-trigger-test-run">🔍 开始分析</button>' +
      '<div id="htyq-trigger-results" style="margin-top:8px;"></div></div>';
    container.appendChild(html);

    document.getElementById('htyq-trigger-test-run') && document.getElementById('htyq-trigger-test-run').addEventListener('click', function() {
      var resultsEl = document.getElementById('htyq-trigger-results');
      if (!resultsEl) return;
      resultsEl.innerHTML = '<div class="sm gray">⏳ 分析中...</div>';

      // 尝试调用世界书模块
      var wb = window._HTYQ_LITE_WORLDBOOK;
      var st = core.loadState();
  ...
  function renderTriggerResults(container, results) {
    ...
    results.forEach(function(r) {
      ...
      html += '<div class="htyq-trigger-result ' + (matched ? 'match' : 'nomatch') + '" style="padding:4px 0;border-bottom:1px solid #21262d;display:flex;justify-content:space-between;align-items:center;">' +
        '<span style="font-size:11px;">' + escHtml(r.book || '未知') + '</span>' +
        '<span class="htyq-trigger-score" style="font-size:10px;color:' + (matched ? '#7ee787' : '#8b949e') + ';">' + Math.round(r.matchScore) + '%</span>' +
      '</div>';
    });
    if (results.length === 0) {
  ...
  function setupBatchBar() {
    var container = document.querySelector('.htyq-lite-panel .tab-content.active');
    if (!container) return;
    var batchBar = document.createElement('div');
    batchBar.className = 'htyq-batch-bar';
    batchBar.id = 'htyq-batch-bar';
    batchBar.innerHTML =
      '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">' +
        '<span class="htyq-batch-count" id="htyq-batch-count" style="font-size:12px;color:#8b949e;">已选 0 项</span>' +
        '<button class="btn btn-sm" id="htyq-batch-select-all">全选</button>' +
        '<button class="btn btn-sm" id="htyq-batch-invert">反选</button>' +
        '<button class="btn btn-sm btn-success" id="htyq-batch-enable">✅ 全部激活</button>' +
        '<button class="btn btn-sm btn-danger" id="htyq-batch-disable">⛔ 全部取消</button>' +
        '<button class="btn btn-sm" id="htyq-batch-export">📤 导出选中</button>' +
      '</div>';
    container.insertBefore(batchBar, container.firstChild);

    // 绑定事件
    setTimeout(function() {
      document.getElementById('htyq-batch-select-all') && document.getElementById('htyq-batch-select-all').addEventListener('click', function() {
        var cbs = container.querySelectorAll('.htyq-wb-entry-cb, .htyq-wb-book-cb');
        cbs.forEach(function(cb) { cb.checked = true; });
        updateBatchCount();
        toast('✅ 已全选');
      });
      document.getElementById('htyq-batch-invert') && document.getElementById('htyq-batch-invert').addEventListener('click', function() {
        var cbs = container.querySelectorAll('.htyq-wb-entry-cb, .htyq-wb-book-cb');
        cbs.forEach(function(cb) { cb.checked = !cb.checked; });
        updateBatchCount();
        toast('🔄 已反选');
      });
      document.getElementById('htyq-batch-enable') && document.getElementById('htyq-batch-enable').addEventListener('click', function() {
        container.querySelectorAll('.htyq-wb-entry-cb:checked').forEach(function(cb) {
          var related = cb.closest('.htyq-wb-entry') || cb.parentNode;
          var toggle = related && related.querySelector('.htyq-wb-entry-active, [data-toggle]');
          if (toggle && toggle.checked !== undefined) toggle.checked = true;
        });
        toast('✅ 已激活选中项');
      });
      document.getElementById('htyq-batch-disable') && document.getElementById('htyq-batch-disable').addEventListener('click', function() {
        container.querySelectorAll('.htyq-wb-entry-cb:checked').forEach(function(cb) {
          var related = cb.closest('.htyq-wb-entry') || cb.parentNode;
          var toggle = related && related.querySelector('.htyq-wb-entry-active, [data-toggle]');
      ...
      document.getElementById('htyq-batch-export') && document.getElementById('htyq-batch-export').addEventListener('click', function() {
        var selected = [];
        container.querySelectorAll('.htyq-wb-entry-cb:checked').forEach(function(cb) {
          var parent = cb.closest('.htyq-wb-entry') || cb.parentNode;
          var nameEl = parent && parent.querySelector('.htyq-wb-entry-name, [data-name]');
        ...
        a.href = url; a.download = 'htyq-wb-export.txt'; a.click();
  ...
  function updateBatchCount() {
    var container = document.querySelector('.htyq-lite-panel .tab-content.active');
    if (!container) return;
    var cbs = container.querySelectorAll('.htyq-wb-entry-cb:checked, .htyq-wb-book-cb:checked');
  ...
  function setupAutoHide() {
    var panel = document.querySelector('.htyq-lite-panel');
    if (!panel) return;
    if (window._htyq_autoHideActive) return;
    window._htyq_autoHideActive = true;

    var hideTimer = null;

    panel.addEventListener('mouseleave', function() {
      if (!window._htyq_autoHideEnabled) return;
      hideTimer = setTimeout(function() {
        panel.classList.add('auto-hide');
        // 显示边缘触发条
        var trigger = document.createElement('div');
        trigger.className = 'htyq-lite-panel.peek-trigger';
        trigger.id = 'htyq-peek-trigger';
        ...
        trigger.addEventListener('mouseenter', function() {
          ...
          var t = document.getElementById('htyq-peek-trigger');
    ...
    if (header) {
      ...
      toggleBtn.id = 'htyq-auto-hide-toggle';
      toggleBtn.style.cssText = 'font-size:10px;padding:2px 6px;margin-left:4px;';
      toggleBtn.textContent = window._htyq_autoHideEnabled ? '👁️ 固定' : '👻 自动隐藏';
      toggleBtn.title = '切换面板自动隐藏模式';
      header.appendChild(toggleBtn);
      toggleBtn.addEventListener('click', function() {
        window._htyq_autoHideEnabled = !window._htyq_autoHideEnabled;
        this.textContent = window._htyq_autoHideEnabled ? '👁️ 固定' : '👻 自动隐藏';
        savePanelState();
        toast(window._htyq_autoHideEnabled ? '👻 自动隐藏已开启' : '👁️ 面板已固定');
      });
    }
  }
  ...
  function renderDashboard(container, state) {
    ...
    var html = '<div class="htyq-dashboard-grid card"><div class="card-title">📊 数据仪表盘</div><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(100px,1fr));gap:6px;">';
    metrics.forEach(function(m) {
      html += '<div class="htyq-dashboard-card" style="background:#0d1117;border-radius:6px;padding:8px 4px;text-align:center;border:1px solid #21262d;">' +
  ...
  function renderUndoHistoryPanel(container, state) {
    ...
    for (var i = history.length - 1; i >= 0; i--) {
      ...
      html += '<div class="htyq-undo-history-item' + (isCurrent ? ' current' : '') + '" style="display:flex;justify-content:space-between;align-items:center;padding:6px 8px;border-bottom:1px solid #21262d;' + (isCurrent ? 'background:rgba(240,192,64,0.08);border-left:3px solid #f0c040;' : '') + '">' +
        '<div>' +
        ...
        '<div>' +
          ...
          (!isCurrent ? '<button class="btn btn-sm htyq-undo-history-rollback" data-idx="' + h.index + '" style="color:#d29922;">↩️ 回滚到此</button>' : '') +
        '</div>' +
      '</div>';
    }
    html += '</div>';
    ...
    setTimeout(function() {
      container.querySelectorAll('.htyq-undo-history-rollback').forEach(function(btn) {
        btn.addEventListener('click', function() {
          var idx = parseInt(this.getAttribute('data-idx'));
          var st = core.loadState();
          if (core.rollbackToUndo(st, idx)) {
  ...
  function setupDeepProfileClick(container, state) {
    ...
    cards.forEach(function(card) {
      card.addEventListener('dblclick', function() {
        ...
        if (detail) {
          var existing = card.querySelector('.htyq-deep-profile-card');
          if (existing) {
  ...
  function setupEntryPreviewClick(container) {
    // 监听由世界书模块渲染的条目
    container.querySelectorAll('.htyq-wb-entry, .wb-entry, [class*="entry"]').forEach(function(el) {
      var nameEl = el.querySelector('.htyq-wb-entry-name, .entry-name, [data-name]');
      if (nameEl && !el._hasPreview) {
        ...
        el.addEventListener('dblclick', function() {
          var entry = {
            ...
            content: el.querySelector('.htyq-wb-entry-content, .entry-content, [data-content]') ? el.querySelector('.htyq-wb-entry-content, .entry-content, [data-content]').textContent.trim() : '',
  ...
  function getClosestBookName(el) {
    var book = el.closest('.htyq-wb-book, .wb-book, [class*="book"]');
    if (book) {
      var title = book.querySelector('.htyq-wb-book-title, .book-title, [data-book-name]');
  ...
  function setupPreviewMode(container, state) {
    var evolveBtn = container.querySelector('[id*="evolve"], [id*="evol"], .htyq-evolve-btn, [class*="evolve"]');
  ...
  function setupMutationObserver() {
    var target = document.querySelector('.htyq-lite-panel');
    if (!target) {
  ...
  function init() {
    ...
    var checkExist = setInterval(function() {
      ...
      if (panel) {
        ...
        setupApiMonitor();
        // 自动隐藏
        if (window._htyq_autoHideEnabled) setupAutoHide();
        // MutationObserver
  ...
  function refreshCurrentTab() {
    window.__htyq_enh_refresh_needed = true;
    enhanceCurrentTab();
  }
