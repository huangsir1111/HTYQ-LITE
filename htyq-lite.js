// htyq-lite.js — 入口：动态加载所有模块，初始化，事件绑定，主API注入
// ============================================================
// ★ 修改记录 ★
// 2026-06-09 v3.0.0 — HTYQ Lite 全员投票集成版
...
//   - 添加防重入锁 window.__HTYQ_INJECTING__
//   - 推演流程异常时自动恢复界面
// 2026-06-05 v2.2.0
...
(function() {
  ...
  const MODULES = [
    'htyq-lite-storage.js',
    'htyq-lite-core.js',
    'htyq-lite-memory.js',
    'htyq-lite-tags.js',
    'htyq-lite-presets.js',
  ...
  var HTYQ_VERSION = '3.1.0';
  function loadScript(src) {
    return new Promise((resolve, reject) => {
      const script = document.createElement('script');
      script.src = src + '?v=' + HTYQ_VERSION;
  ...
  function reloadCSS() {
    var links = Array.from(document.querySelectorAll('link[rel=stylesheet]')).filter(function(l) { return l.href && l.href.toLowerCase().indexOf('htyq-lite') >= 0; });
    links.forEach(function(link) {
      var oldHref = link.href.split('?')[0];
      link.href = oldHref + '?v=' + HTYQ_VERSION;
    });
  }
  ...
  function registerInjection(content) {
    try {
      ...
      console.error('[HTYQ Lite] 注入失败', e);
      return false;
    }
  }
  ...
  async function init() {
    ...
    console.log('[HTYQ Lite] 加载基础路径:', baseUrl);
    try {
      for (const mod of MODULES) {
        await loadScript(`${baseUrl}/${mod}`);
        console.log(`[HTYQ Lite] 已加载: ${mod}`);
        // ★ 运行时检查：验证模块全局变量已定义，防止语法错误静默失败
        const modVar = 'HTYQ_LITE_' + mod.replace('htyq-lite-', '').replace('.js', '').toUpperCase();
        if (!window[modVar]) {
          console.error(`[HTYQ Lite] ❌ ${mod} 加载异常：${modVar} 未定义，文件可能存在语法错误`);
        }
      }
      ...
      async function beforeMessageSend() {
        ...
        try {
          ...
          if (!worldbookLoaded) {
            ...
            try { await worldbook.loadWorldbooks(); } catch(e) { console.warn('[HTYQ] worldbook 模块失败', e); }
          }
          ...
          if (driveMode === 'manual' && currentRound > 0 && currentRound % evolveInterval === 0) {
            ...
            try {
              ...
              removePersistToast();
              console.warn('[HTYQ Lite] 手动模式推演失败', e);
            }
          }

          let tags; try { tags = await tagsGen.generatePredictionTags(chatHistory, state); } catch(e) { console.warn('[HTYQ] tags 模块失败', e); tags = []; }
          let context; try { context = await inject.buildContext(chatHistory, state, tags, { includeWorldbook: settings.injectWorldbook === true }); } catch(e) { console.warn('[HTYQ] inject 模块失败', e); context = ''; }
          ...
          if (success) {
            console.log(`[HTYQ Lite] 注入成功 (round ${currentRound}, ${context.length} chars)`);
          } else {
            ...
            if (typeof ctx.addOneMessage === 'function') {
              ...
              console.warn('[HTYQ Lite] 无法注入上下文');
            }
          }
        } finally {
      ...
      async function onMessageReceived() {
        ...
        try {
          ...
          if (chat.length <= 2) {
            console.log('[HTYQ Lite] 新聊天冻结：等待第一轮完整交互（chat.length=' + chat.length + '）');
            isEvolving = false;
            removePersistToast();
            ui.refresh();
            return;
          ...
          // 存储本轮记忆
          try { await memory.storeMemoryFromRound(state, userMsg, aiMsg, state.round); } catch(e) { console.warn('[HTYQ] memory 模块失败', e); }

          // 强制触发事件链 + 血仇追杀
          try { evolution.forceTriggerEvents(state); } catch(e) { console.warn('[HTYQ] evolution 事件链模块失败', e); }
          try { evolution.advanceBloodFeud(state); } catch(e) { console.warn('[HTYQ] evolution 血仇模块失败', e); }
          ...
          if (evolveSuccess && timeModule && typeof timeModule.calculateTimeIncrement === 'function') {
            ...
            let increment; try { increment = timeModule.calculateTimeIncrement(evolveResult, chatText, settings); } catch(e) { console.warn('[HTYQ] time 模块失败', e); increment = 0; }
            ...
            if (triggered.includes('events')) {
              console.log('[HTYQ Lite] ⏰ 时间到达事件链触发阈值');
            }
            if (triggered.includes('chapter')) {
              console.log('[HTYQ Lite] 📖 时间到达章节摘要触发阈值');
              // 取本轮之前最近 10 轮作为章节范围
            ...
            if (triggered.includes('volume')) {
              console.log('[HTYQ Lite] 📚 时间到达卷摘要触发阈值');
          ...
          if (core && typeof core.autoBackup === 'function') {
            try { core.autoBackup(core.loadState()); } catch(e) { console.warn('[HTYQ] 自动备份失败', e); }
          }
          ui.refresh();
        } catch (e) {
          console.error('[HTYQ Lite] 处理失败', e);
          removePersistToast();
      ...
      async function onChatLoaded() {
        ...
        if (chat.length === 0) {
          ...
          if (typeof worldbook !== 'undefined' && worldbook.loadWorldbooks && worldbook.getAvailableBooks) {
            worldbook.loadWorldbooks().then(function() { }).catch(function(e) { console.warn('[HTYQ Lite] 空聊天预加载世界书失败', e); });
          }
        ...
        const state = core.loadState();
        ui.refresh();
        console.log('[HTYQ Lite] 聊天已加载，世界书同步完成');
      }
          ...
          try {
            ...
            if (savepoints.length > 0) {
              ...
              if (chatLen > 0) {
                ...
                if (restored) {
                  console.log('[HTYQ Lite] 消息切换，世界状态已回退 (chatLen=' + chatLen + ')');
                  if (ui && ui.refresh) ui.refresh();
                }
              }
            }
          } catch(e) {
            console.warn('[HTYQ Lite] 状态回退失败', e);
          }
        }
        console.log('[HTYQ Lite] 检测到消息切换，已重置注入标记');
      }
        ...
        try {
          ...
          if (core && core.rollbackToChatLength && core.getSavepoints) {
            ...
            if (restored) {
              ...
              localStorage.setItem(core.getSavepointKey(), JSON.stringify(sps));
              if (ui && ui.refresh) ui.refresh();
            } else {
              // Bug-001 fix: 不删存档，只警告
              console.warn('[HTYQ Lite] 未找到匹配 chatLen=' + chatLen + ' 的 savepoint，保留现有存档');
            }
          }
        } catch(e) {
          console.warn('[HTYQ Lite] onMessageDeleted 处理失败', e);
        }
      }
      ...
      if (ctx && ctx.eventSource) {
        ...
        console.log('[HTYQ Lite] 事件绑定成功');
      } else {
        console.warn('[HTYQ Lite] 无法绑定事件，自动推演和注入不可用');
      }
      ...
      console.error('[HTYQ Lite] 初始化失败', err);
    }
  }
