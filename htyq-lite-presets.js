// htyq-lite-presets.js — 预设系统：4 种内置预设 + CRUD + 存储 + 导入导出
// ============================================================
// v2.5.0 — 新增模块
// ============================================================
...
window.HTYQ_LITE_PRESETS = (function() {
  'use strict';
  ...
  // ==================== 存储 ====================
  var STORAGE_KEY = 'htyq_lite_presets';
  var ACTIVE_KEY = 'htyq_lite_active_preset';

  function loadPresetsFromStorage() {
    try {
      var raw = localStorage.getItem(STORAGE_KEY);
      if (raw) return JSON.parse(raw);
    } catch(e) { console.warn('[HTYQ Presets] 读取存储失败', e); }
    return null;
  }

  function savePresetsToStorage(presets) {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(presets)); }
    catch(e) { console.warn('[HTYQ Presets] 保存失败', e); }
  }
  ...
  function setActivePreset(presetId) {
    ...
    try { localStorage.setItem(ACTIVE_KEY, presetId); } catch(e) { return false; }
    return true;
  }
  ...
  function deletePreset(presetId) {
    ...
    if (activeId === presetId) {
      try { localStorage.setItem(ACTIVE_KEY, 'standard'); } catch(e) {}
    }
  ...
  function cleanContext(text, excludeRanges) {
    ...
    for (var i = 0; i < excludeRanges.length; i++) {
      ...
      try {
        ...
        console.warn('[HTYQ Presets] 排除规则正则失败:', rule, e);
      }
    }
    return cleaned;
  }
  ...
  function getInjectStyle() {
    try { return localStorage.getItem('htyq_inject_style') || 'standard'; } catch(e) { return 'standard'; }
  }

  function setInjectStyle(style) {
    if (!INJECT_STYLES[style]) return false;
    try { localStorage.setItem('htyq_inject_style', style); return true; } catch(e) { return false; }
  }
