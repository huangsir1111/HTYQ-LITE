// htyq-lite-slash.js — Slash命令注册（完整版 v2.1.0）
// ============================================================
// ★ 修改记录 ★
// 2026-06-05 v2.1.0
//   - 添加 /world 命令（切换面板、状态、手动推演）
//   - 添加 /memory 命令（召回、摘要、统计）
//   - 添加 /htyq status 命令（整体状态汇总）
//   - 添加 /htyq evolve 命令（手动推演变体）
//   - 增强错误提示和用法说明
...
window.HTYQ_LITE_SLASH = (function() {
  ...
  function registerCommands() {
    const ctx = (typeof SillyTavern !== 'undefined' && SillyTavern.getContext) ? SillyTavern.getContext() : null;
    if (!ctx) {
      console.warn('[HTYQ Lite] 无法获取上下文，slash命令未注册');
      return;
    }
    ...
    if (typeof registerSlashCommand === 'function') {
      ...
      console.log('[HTYQ Lite] slash命令已注册 (/world, /memory, /htyq)');
      ...
      console.warn('[HTYQ Lite] slash命令通过 ctx.slashCommand.register 注册（非标准API）');
    } else {
      console.warn('[HTYQ Lite] 未找到slash命令注册接口');
    }
  }
